(function($) {
  ...
  var cloudSaveTimeout;
  
  var autoClickerDuration;
  var autoClickerInterval;
  var powerShotDuration;
  var specialForceDuration;
  var luckyCoinDuration;
  ...
  var initGame = function() {
	...
	if(backDateStatus == "story"){
	  ...
	  if(hero.userName != ""){
		...
		showSideMenu();
		updateUserName();
		setting.autoProgress = hero.settingOfflineProgress;
		updateData();
		...
		if(hero.highestFloor >= setting.unlockReqFloorHouse){
		  calculateGenerateTokenInBank();
		}
		if(hero.highestFloor >= setting.unlockReqFloorBarracks){
		  calculateAllPartnerAbility();
		}
		if(hero.highestFloor >= setting.unlockReqFloorCompanion){
		  insertAvailableCompanion();
		  calculateAllCompanionDamage();
		}
		if(hero.highestFloor >= setting.unlockReqFloorEquipment){
		  calculateAllItemBenefit();
		}
		if(hero.highestFloor >= setting.unlockReqFloorGods){
		  insertAvailableGods();
  ...
  var exportData = function (isDownload=false){
	let exportSaveData = '';
	exportSaveData = CryptoJS.AES.encrypt(JSON.stringify(hero), setting.keyGame).toString();
	
	if(isDownload){
	  let fileName = "KID-"+hero.userId+"-"+hero.userName;
  ...
  var importData = function (){
	...
	let bytes  = CryptoJS.AES.decrypt(importData, setting.keyGame);
	let decryptSaveData;
	try {
      decryptSaveData = JSON.parse(bytes.toString(CryptoJS.enc.Utf8));
	  localStorage.setItem("hero", JSON.stringify(decryptSaveData));
	  hero = JSON.parse(localStorage.getItem("hero"));
	  saveGame();
  ...
  var localSaveGame = function () {
	...
	if(hero.gameVersion.localeCompare('0.7b-alpha') < 0) hero.showDevLog = false;
	if(hero.showDevLog == false){
	  if(!($("#modalOfflineForm").data('bs.modal') || {})._isShown && !($("#modalRewardAdventureConfirm").data('bs.modal') || {})._isShown && !($("#modalListRewardAdventureConfirm").data('bs.modal') || {})._isShown && !($("#modalDailyCheckIn").data('bs.modal') || {})._isShown && !($("#modalNotification").data('bs.modal') || {})._isShown){
		...
		if(hero.tutorialGroupId == 0 && new Decimal(hero.totalGainOrbs).greaterThan(0)) hero.tutorialGroupId = -1;
	...
	if(hero.dailyTask.length > 0)updateTask(11, onlineDuration);
  ...
  var checkCssSetting = function(){
	...
	switch(hero.settingBackground){
	  case "light":
		...
		break;
	  case "dark":
	    $('body').css({"color":"#FFFFFF", "background-color":"#000000"});
		$('.card,.drop-down-list').css({"color":"#FFFFFF", "background-color":"#000000"});
		$('.card-header').css({"color":"#FFFFFF", "background-color":"#333333"});
		$('.unactive-bg-color').css({"background-color":"#000000", "color":"#FFFFFF"});
		...
		$('.tab-button-unactive').css({"border":"1px solid #FFFFFF","background-color":"#555555"});
		$('.upgrade-level-button,.shop-button-class,.dpc-button').css({"color":"#FFFFFF", "background-color":"#555555"});
		$('.modal-content,.form-control').css({"background-color":"#000000", "color":"#FFFFFF"});
		$('.unlock-info,.tutor-info').css({"color": "#FFB3A7"});
		$('#content-wrapper').css({"background-color":"#000000"});
  ...
  var resetGame = function (isPrestige=false){
	...
	if(isPrestige){
	  ...
	  for(let i=0;i<hero.unlockPet.length;i++){
		...
		switch(unlockPet.id){
		  ...
		  default:
			...
			totalDps = totalDps.plus(nextDps).floor();
			hero.dps = totalDps;
			hero.countPartner++;
			break;
		}
	  ...
	  for(let i=0;i<hero.countLabor;i++){
		...
		if(unlockLabor.level > 0){
		  unlockLabor.level = 1;
		  unlockLabor.currentPrize = unlockLabor.baseFee;
		}
	  }
  ...
  var updateData = function (){
	if(hero.highestPartnerUnlock){
	  delete hero.highestPartnerUnlock;
	  hero.highestUnlockPartner = [];
	}
	...
	if(hero.activeGodsSlot == 0 && hero.highestFloor >= setting.unlockReqFloorGods) hero.activeGodsSlot = 1;
	if(hero.activeTrainingSlot == 0 && hero.highestFloor >= setting.unlockReqFloorBarracks) hero.activeTrainingSlot = 1;
	if(hero.activeCompanionSlot == 0 && hero.highestFloor >= setting.unlockReqFloorCompanion) hero.activeCompanionSlot = 1;
	if(hero.activeEquipmentSlot == 0 && hero.highestFloor >= setting.unlockReqFloorEquipment) hero.activeEquipmentSlot = 1;
  ...
  var updateDataAfterUpdate = function(){
	...
	if(hero.gameVersion.localeCompare('0.6-alpha') < 0){
	  ...
	  hero.unlockDwellers = [];
	  if(hero.highestFloor >= setting.unlockReqFloorBarracks) unlockNewDwellers(hero.activeTrainingSlot,"trainer",true,true);
	  if(hero.highestFloor >= setting.unlockReqFloorCompanion) unlockNewDwellers(hero.generatedCompanionSlot,"warlords",true,true);  
	  let petData = pet.find(item => item.id === 0);
	  if(hero.highestFloor >= setting.unlockReqFloorGods) unlockNewDwellers(hero.activeGodsSlot,"cleric",true,true);
	}
	...
	if(hero.gameVersion.localeCompare('0.7b-alpha') < 0){
	  ...
	  if(hero.highestFloor >= setting.unlockReqFloorLabor) insertNewUnlockLabor();
  ...
  var firstTimeVisit = function () {
	...
	hideSideMenu();
	hideAllContent();
	if(hero.userName == "") $("#modalLoginForm").modal('show');
  };
  ...
  var showStory = function(type,seqNumber){
	if(seqNumber == "finish"){
	  ...
	  let currentStory = story.find(item => item.type === type && item.seqNumber === seqNumber);
	  let countStory = story.filter(item => item.type === type).length;
	  ...
	  switch(currentRole.toLowerCase()){
	    case "faustina":
		  ...
		  $("#queen-img").attr("src", "assets/images/queen.svg");
	    ...
	    case "kuro":
	    default:
	      if(currentRole.toLowerCase() == "kuro") $("#king-img").attr("src", "assets/images/king.svg");
	      else $("#king-img").attr("src", "assets/images/king-user.svg");
	  ...
	  if(bodyDiv != ""){
	    ...
	    if(seqNumber == 1) componentAction("prev-story-button","addClass","d-none");
	    else{
	      componentAction("prev-story-button","removeClass","d-none");
  ...
  var skipStory = function(){
	if(hero.kingdomName == "") showStory("prolog",10);	
  ...
  var formulaBossTimer = function(){
	let maxBossTimer = ((setting.baseBossTimeLimit * 1) + (hero.bonusBossTimeLimit * 1));
	return maxBossTimer;
  };
  ...
  var stopTowerBossTimer = function(){
	...
	if(unlockTower){
	  if(currentTowerManagerHp.lessThanOrEqualTo(0) && restTowerBossTimer > 0){
	    ...
	    showDetailTowerArea("tower-area-"+selectedTowerId,true);
  ...
  var stopBossTimer = function(){
	if($("#timer-bar").hasClass("timer-bar-bgcolor")) $("#timer-bar").removeClass("timer-bar-bgcolor");
	hero.floor--;
  ...
  var loopBossTimer = function(){
	...
	while(interval >= (newBossInterval*2)){
  ...
  var initEnemy = function(){
	let baseEnemyHP = new Decimal(setting.enemyHPMultiplier).pow(hero.floor-1).mul(enemy.baseHp);
  ...
  var checkAction = function(showAnimation=false,idleLoop=1) {
	...
	if(idleLoop == 1){
	  ...
	  if(hero.maxFloor == setting.unlockReqFloorPrestige && hero.totalGainOrbs == 0 && notifCanPrestige == true) {
	    updateTab('hero');
	...
	if(unlockDailyTask && unlockDailyTask.count < dailyTaskRequired){
	  switch(taskId){
	    ...
	    case 11:
		  ...
		  break;
		default:
		  unlockDailyTask.count = unlockDailyTask.count + addCount;
		  break;
	  }
	...
	let testTimeout = setTimeout (function() {
	  checkLoopAction(newDpsInterval, function(){
	    ...
	    runBackgroundCalculation(false, function(){
	      ...
	      loopDpsTimer();
		  startEverySecInterval();
		  if(hero.floor % enemy.bossFloor == 0 && !bossTimeout) loopBossTimer();
	    });
	  });
	},25);
  });
	  ...
	  for(let i=0; i<arrayComponentId.length; i++){
	    switch (action) {
		  ...
		  case 'replaceText':
		    ...
		    $("#"+arrayComponentId[i]).text(actionValue);
		    break;
		  case 'inputText':
  ...
  var calculateAllArtifactDamage = function(firstRun=false){
	...
	for(let i=0; i<hero.countArtifact; i++){
	  ...
	  artifactDamage = artifactDamage.plus(artifactData.tier * unlockArtifact.level * 5);
  ...
  var calculateAllCompanionDamage = function(){
	...
	for(let i=0;i<hero.countCompanion;i++){
	  ...
	  if(unlockCompanion.active){
	    if(unlockCompanion.id < 15) companionDpcPercentage += unlockCompanion.damage;
	    else companionDpsToDpcPercentage = unlockCompanion.damage;
	  }else{
		if(unlockCompanion.id < 15) companionDpcPercentage += Math.floor(unlockCompanion.level / setting.passiveCompanionCalculateLevel) * setting.passiveCompanionCalculateLevel * companionData.damage * setting.passivePercentage / 100;
	    else companionDpsToDpcPercentage = Math.floor(unlockCompanion.level / setting.passiveCompanionCalculateLevel) * setting.passiveCompanionCalculateLevel * companionData.damage * setting.passivePercentage / 100;
	  }
  ...
  var calculateAllPartnerAbility = function(){
	for(let i=0;i<hero.highestUnlockPartner.length;i++){
	  let highestUnlockPartner = hero.highestUnlockPartner[i];
	  let partnerId = highestUnlockPartner.id;
	  let activeAbility = -1;
	  ...
	  for(let a=0;a<highestUnlockPartner.ability.length;a++){
		...
		if(unlockAbilityPartner.isActive){
		  ...
		  interval = unlockAbilityPartner.interval;
  ...
  var formulaPartnerWeaponDamage = function(partnerId){
	let partnerData = partner.find(item => item.id === partnerId);
	let bonusWeaponPercentage = new Decimal(100).plus(hero.bonusAllDamagePercentage);
  ...
  var calculateAllItemBenefit = function(){
	  ...
	  for(let i=0;i<battlingPartner.length;i++){
		...
		if(unlockItem){
		  switch(unlockItem.itemType.toLowerCase()){
		    case "weapon":
			  let bonusWeaponPercentage = new Decimal(formulaPartnerWeaponDamage(battlingPartner[i].id));
			  damageMinValue = new Decimal(10).pow(unlockItem.exponentValue).mul(unlockItem.minValue).mul(bonusWeaponPercentage).plus(damageMinValue);
			  damageMaxValue = new Decimal(10).pow(unlockItem.exponentValue).mul(unlockItem.maxValue).mul(bonusWeaponPercentage).plus(damageMaxValue);
		      break;
		  };
		}
	  }
  ...
  var calculateGenerateTokenInBank = function(){
	...
	if(totalSec >= (setting.bankTokenGenerateHour * 3600)){
	  ...
	  hero.tokenInBank += generateToken;
  ...
  var updateHeroCoin = function() {
	...
	componentAction ("hero-coin", "replaceText", numberFormat(heroCoin));
	updateAchievement(2);
  };
  
  var updateHeroDpc = function(currentDpc=0) {
	if(currentDpc==0) componentAction ("hero-dpc", "replaceText", calcTotalDpc());
	else componentAction ("hero-dpc", "replaceText", numberFormat(currentDpc));
	componentAction ("current-hero-dpc", "replaceText", hero.dpc);
	componentAction ("next-hero-dpc", "replaceText", calcNextHeroDpc());
  ...
  var updateHeroDps = function() {
	let minValue = new Decimal(hero.equipmentMinValue);
	let exponentValue = new Decimal(hero.equipmentExponentValue);
	let weaponDamage = new Decimal(0);
	let totalDps = new Decimal(hero.totalDps);
	if(minValue.greaterThan(0) && exponentValue.greaterThan(0)){
	  weaponDamage = new Decimal(10).pow(exponentValue).mul(minValue);
	  totalDps = totalDps.plus(weaponDamage);
	}
  ...
  var updateHeroOrbs = function() {
	componentAction ("hero-orbs", "replaceText", hero.orbs);
	updateAchievement(3);
  };
  
  var updateHeroLevel = function() {
	componentAction ("hero-level", "replaceText", hero.heroLevel);
  };
  
  var updateWeaponLevel = function() {
	componentAction ("weapon-level", "replaceText", hero.weaponLevel);
  };
  
  var updateWeaponDpc = function() {
	if(hero.weaponLevel > 0){
	  componentAction ("upgrade-weapon-text", "replaceText", "Level Up x");
	  
	  if(setting.levelUpgradeMultiplier > 0) componentAction ("level-weapon-upgrade-multiplier", "replaceText", setting.levelUpgradeMultiplier);
	  else if(setting.levelUpgradeMultiplier == 0) componentAction ("level-weapon-upgrade-multiplier", "replaceText", maxWeaponLevelBuy);
	}	
	componentAction ("current-weapon-dpc", "replaceText", hero.weaponDpc);
	componentAction ("next-weapon-dpc", "replaceText", calcNextWeaponDpc());
  };
  
  var updateCompanion = function(){
	if(hero.highestFloor < setting.unlockReqFloorCompanion){
	  componentAction ("hero-companion-unlock-div", "removeClass", "d-none");
	  componentAction ("companion-unlock-requirement", "replaceText", setting.unlockReqFloorCompanion);
	...
	for(let i=0; i<hero.countPartner; i++){
	  ...
	  if(unlockPartner){
		...
		if(unlockPartner.level > 0){
	      componentAction ("upgrade-partner-text-"+partnerId, "replaceText", "Level Up x");
		  if(setting.levelUpgradeMultiplier > 0)componentAction ("level-partner-upgrade-multiplier-"+partnerId, "replaceText", setting.levelUpgradeMultiplier);
		  else if(setting.levelUpgradeMultiplier == 0){
		    let localMaxPartnerLevelBuy = maxPartnerLevelBuy.find(item => item.id === partnerId);
	        if(localMaxPartnerLevelBuy) componentAction ("level-partner-upgrade-multiplier-"+partnerId, "replaceText", localMaxPartnerLevelBuy.maxLevelBuy);
		  }
		  componentAction ("partner-level-"+partnerId, "replaceText", unlockPartner.level);
		  if(unlockPartner.critHit) componentAction ("current-partner-dps-"+partnerId, "replaceText", calcTotalPartnerDps(partnerId,"current",true,true));
		  else componentAction ("current-partner-dps-"+partnerId, "replaceText", calcTotalPartnerDps(partnerId,"current"));
		}
	  }
	  componentAction ("next-partner-dps-"+partnerId, "replaceText", calcTotalPartnerDps(partnerId,"next"));
	}
  };
  
  var updateLabor = function() {
	for(let i=0; i<hero.countLabor; i++){
	  ...
	  if(unlockLabor){
		if(unlockLabor.level > 0){
		  if(unlockLabor.level == setting.maxLaborLevel){
			componentAction("upgrade-labor-button-"+i,"addClass","d-none");
	      }else{
			componentAction ("upgrade-labor-text-"+i, "replaceText", "Level Up x");
			if(setting.levelUpgradeMultiplier > 0){
			  if(unlockLabor.level + setting.levelUpgradeMultiplier < setting.maxLaborLevel) componentAction ("level-labor-upgrade-multiplier-"+i, "replaceText", setting.levelUpgradeMultiplier);
			  else{
				let restLevel = setting.maxLaborLevel - unlockLabor.level;
				componentAction ("level-labor-upgrade-multiplier-"+i, "replaceText", restLevel);
			  }
		    }else if(setting.levelUpgradeMultiplier == 0){
		      let localMaxLaborLevelBuy = maxLaborLevelBuy.find(item => item.id === i);
	          if(localMaxLaborLevelBuy) componentAction ("level-labor-upgrade-multiplier-"+i, "replaceText", numberFormat(localMaxLaborLevelBuy.maxLevelBuy));
		    }
		  }
	  ...
	  if(unlockLabor.level < setting.maxLaborLevel) componentAction ("next-labor-prize-"+i, "replaceText", " (+"+numberFormat(calcTotalLaborPrize(i,"next"))+")");
	  else componentAction ("next-labor-prize-"+i, "replaceText", "");
	}
	...
	for(let i=0; i<hero.countSkill; i++){
	  ...
	  if(unlockSkill && unlockSkill.level > 0){
	    if(unlockSkill.level > 0){
		  ...
		  $("#skill-image-"+i).attr("src", "assets/images/levelup.svg");
  ...
  var updateUpgradeLevelFeeHero = function() {
	...
	componentAction ("upgrade-level-fee", "replaceText", numberFormat(totalUpgradeFee.ceil()));	
  ...
  var updateUpgradeLevelFeeWeapon = function() {
	...
	componentAction ("upgrade-weapon-fee", "replaceText", numberFormat(totalUpgradeFee.ceil()));
  ...
  var formulaUpgradePartnerFee = function(partnerLevel, basePrice){
	let upgradeFee = new Decimal(setting.partnerUpgradeCoinMultiplier).pow(partnerLevel).mul(basePrice);
  ...
  var updateUpgradeLevelFeePartner = function(partnerId) {
	if(partnerId >= 0){	
	  let totalUpgradeFee = new Decimal(0);  
	  let loopNextLevel = 0;
	  let unlockPartner = hero.unlockPartner.find(item => item.id === partnerId);
	  ...
	  componentAction ("upgrade-partner-fee-"+partnerId, "replaceText", numberFormat(totalUpgradeFee.ceil()));
  ...
  var updateUpgradeLevelFeeLabor = function(laborId) {
	if(laborId >= 0){	
	  ...
	  if(loopNextLevel > 0){
	    for(let a=0; a<loopNextLevel; a++){
		  let upgradeFee = new Decimal(unlockLabor.baseFee).mul(unlockLabor.level+a+1);
		  totalUpgradeFee = totalUpgradeFee.plus(upgradeFee);
	    }
	  }else{
		if(setting.levelUpgradeMultiplier == 0){
		  let c=0;
		  let currentLevel = unlockLabor.level;
		  do{
			let upgradeFee = new Decimal(unlockLabor.baseFee).mul(currentLevel+c+1);
		    nextTotalUpgradeFee = totalUpgradeFee.plus(upgradeFee);
	      ...
	      }while(nextTotalUpgradeFee.lessThanOrEqualTo(heroCoin) && (currentLevel+c) < setting.maxLaborLevel);
		  let localMaxLaborLevelBuy = maxLaborLevelBuy.find(item => item.id === laborId);
		  if(localMaxLaborLevelBuy) localMaxLaborLevelBuy.maxLevelBuy = c;
	    }
	  }
	  
	  totalUpgradeFee = new Decimal(100).sub(hero.bonusCoinLaborUpgradeFee).div(100).mul(totalUpgradeFee).ceil();
	  if(unlockLabor.level < setting.maxLaborLevel) componentAction ("upgrade-labor-fee-"+laborId, "replaceText", numberFormat(totalUpgradeFee));
  ...
  var showGenerationTimer = function(){
	if(hero.highestFloor >= setting.unlockReqFloorCompanion){
	  if($.trim( $('#companion-generate').text() ).length == 0){
	    ...
	    appendCompanionIcon += '<div class="col-xl-12 col-lg-12 col-md-12 col-sm-12 col-12 no-gutters v-center-flex">';
	    appendCompanionIcon += '<span id="companion-note" class="unlock-info"></span>';
	    appendCompanionIcon += '</div>';
	    appendCompanionIcon += '</div>';
	  ...
	  if(hero.filledCompanionSlot > 0) showGenerateNote("companion");
	  else{
		componentAction("companion-note","clearText","");
		componentAction("companion-generate","removeClass","cursor-button");
	  }
	}else componentAction("companion-generate","addClass","d-none");
	...
	}else componentAction("equipment-generate","addClass","d-none");
  };
  ...
  var checkCollectDailyCheckIn = function(){
	...
	if(hero.lastCheckInDateTime == 0) canCollectDailyCheckIn = true;
	else{
  ...
  var showDailyCheckInArea = function(){
	...
	if( !$( "#list-daily-check-in" ).length){
	  ...
	  appendDailyCheckInDiv += '<div class="col-xl-12 col-lg-12 col-md-12 col-sm-12 col-12 middle-align mb-2">VIP Level : '+hero.checkInLevel+'</div>';
	  for(let i=0;i<hero.dailyCheckIn.length;i++){
	    ...
	    appendDailyCheckInDiv += '<div id="daily-check-in-' + i + '" class="row col-xl-12 col-lg-12 col-md-12 col-sm-12 col-12 rounded-card m-0">';
	  ...
	  $("#daily-check-in-div").append(appendDailyCheckInDiv);
	}else{
	  for(let i=0;i<hero.dailyCheckIn.length;i++){
		let heroDailyCheckIn = hero.dailyCheckIn[i];
	...
	if(checkCollectDailyCheckIn()) $("#daily-check-in-collect").text("Collect");
  ...
  var checkCompleteDailyCheckIn = function(){
	...
	if(completedDailyCheckIn == 30 && hero.countCompleteCheckIn == 30 && hero.countCompleteCheckIn > 0) return true;
	else return false;
  };
  ...
  var insertNewUnlockItem = function(materialId,qty,itemType,name,partnerType,storageType){
  ...
  var showOrbsArea = function(){
	...
	if(heroOrbs.greaterThan(0) || hero.unlockArtifact.length > 0){
	  componentAction("hero-orbs-div", "removeClass", "d-none");
	  componentAction ("hero-orbs-title", "replaceText", "Orbs : ");
	  updateHeroOrbs();
	}  
  };
  ...
  var showBodyArea = function(){
	if(setting.heroAreaShow == 1 || hero.coin >= setting.unlockRequirementCoinForHero || hero.heroLevel > 1){
		...
		componentAction ("hero-user-name", "replaceText", hero.userName);
		updateHeroLevel();
	    updateCompanion();
		if(setting.levelUpgradeMultiplier > 0) componentAction ("level-hero-upgrade-multiplier", "replaceText", setting.levelUpgradeMultiplier);
		else if(setting.levelUpgradeMultiplier == 0) componentAction ("level-hero-upgrade-multiplier", "replaceText", maxHeroLevelBuy);
		componentAction ("unlock-weapon-requirement", "replaceText", setting.unlockRequirementLevelForWeapon);
	}
	componentAction ("unlock-partner-requirement", "replaceText", partner[0].unlockReq);
  };
  
  var showWeaponArea = function(){
	if(hero.weaponAreaShow == 1 || hero.heroLevel >= setting.unlockRequirementLevelForWeapon){
		componentAction ("weapon-unlock-div", "removeClass", "d-none");
	...
	for(let i = 0; i<partner.length; i++){
	  ...
	  if(currentLevel >= nextUnlockPartnerData.unlockReq){
		...
		if( !$( "#partner-area-" + i ).length){
		  ...
		  nextPartnerDPS = calcTotalPartnerDps(i,"next");
		...
		let appendPartnerDiv = '<div id="partner-unlock-div" class="rounded-card middle-align unlock-info">Unlock Partner at ' + nextUnlockPartnerData.prevPartnerName + ' level ' + nextUnlockPartnerData.unlockReq + '</div>';
  ...
  var showSkillArea = function(){
	...
	if(hero.unlockSkill.length > 1 && !$( "#start-all-skill").length){
	  let appendStartAllDiv = '<div id="start-all-skill" class="row border-top rounded-card p-0 border-0">';
	  appendStartAllDiv += '<button id="start-all-skill-button" class="use-skill-button skill-active">Start All Skill</button>';
	...
	for(let i=0; i<skill.length; i++){
	  ...
	  if(unlockReqFloor >= skill[i].unlockReqFloor){
		...
		if( !$( "#skill-area-" + i ).length){
		  ...
		  appendSkillDiv += '<div id="skill-name-'+i+'" class="col-xl-4 col-lg-4 col-md-4 col-sm-6 col-6 v-center-flex">';
		  appendSkillDiv += skillData.name + ' Lv <span id="skill-level-' + i + '">' + skillLevel + '</span>';
		  appendSkillDiv += '</div>';
		  appendSkillDiv += '<div class="col-xl-4 col-lg-4 col-md-4 col-sm-6 col-6 v-center-flex">';
		  ...
		  appendSkillDiv += '</div>';
		  appendSkillDiv += '<div class="col-xl-4 col-lg-4 col-md-4 col-sm-12 col-12">';
		  ...
		  $("#skill-area-div").append(appendSkillDiv).on('click','#upgrade-skill-button-'+i,function(){
			upgradeLevelSkill(this); 
		  });
		  
		  appendSkillDiv = '<div id="skill-use-div-' + i + '" class="col-xl-12 col-lg-12 col-md-12 col-sm-12 col-12 mt-2 ' + styleDivBtnUseSkill + '">';
		...
		if(i+1 < skill.length){
		  let appendSkillDiv = '<div id="skill-unlock-div" class="rounded-card middle-align unlock-info">Unlock Next Skill at Floor ' + skill[i].unlockReqFloor + '</div>';
  ...
  var calcTotalProduceLaborCoin = function(){
	...
	totalLaborPrize = new Decimal(100).plus(hero.bonusCoinLaborYield).plus(hero.bonusAllCoinPercentage).div(100).mul(totalLaborPrize);
  ...
  var produceLaborCoin = function(idleLoop=1){
	...
	dungeonBattleLog = '<img src="assets/images/coin-labor.svg" class="image-size"></img>&nbsp;Coin Labor Yield '+numberFormat(totalLaborPrize)+'&nbsp;<img src="assets/images/coin.svg" class="image-size-2"></img><br/>';
  ...
  var showLaborArea = function(){
	if(hero.highestFloor >= setting.unlockReqFloorLabor){
	  let appendLaborDiv = "";	  
	  for(let i=0;i<hero.countLabor;i++){
		let unlockLabor = hero.unlockLabor[i];
		...
		if(unlockLabor && unlockLabor.level > 0){
		  upgradeFee = updateUpgradeLevelFeeLabor(i);
		  upgradeText = 'Level Up x';
		  if(unlockLabor.level >= setting.maxLaborLevel){
			addLaborText = "";
		  }
		...
		if( !$( "#labor-area-" + i ).length){
		  ...
		  if(availableLabor){
		    ...
		    appendLaborDiv += labor[i] + '&nbsp;-&nbsp;Lv&nbsp;<span id="labor-level-' + i + '">' + unlockLabor.level + '</span>';
		    appendLaborDiv += '</div>';
		    appendLaborDiv += '<div class="col-xl-4 col-lg-4 col-md-4 col-sm-6 col-6 v-center-flex">';
		    ...
		    appendLaborDiv += '<button id="upgrade-labor-button-' + i + '" class="upgrade-level-button upgrade-labor" value="' + i + '"><img src="assets/images/levelup.svg" class="image-size-2">&nbsp;<span id="upgrade-labor-text-' + i + '">' + upgradeText + '</span><span id="level-labor-upgrade-multiplier-' + i + '">' + upgradeMultiplier + '</span> - <span id="upgrade-labor-fee-' + i + '">' + numberFormat(upgradeFee) + '</span> Coin</button>';
		    appendLaborDiv += '</div>';
		    appendLaborDiv += '</div>';
	  ...
	  if( !$( "#labor-unlock-div").length){
	    let appendLaborDiv = '<div id="labor-unlock-div" class="rounded-card unlock-info middle-align">Unlock at Floor '+setting.unlockReqFloorLabor+'</div>';
  ...
  var showShopArea = function(){
	...
	for(let i=0;i<cleanShop.length;i++){
	  ...
	  if( !$( "#shop-area-" + i ).length){
		...
		appendShopDiv += '<div id="shop-area-' + i + '" class="row border-top rounded-card">';
	    appendShopDiv += '<div class="col-xl-12 col-lg-12 col-md-12 col-sm-12 col-12">';
		...
		for(let a=0;a<arrayShopData.length;a++){
		  ...
		  if(arrayShopData[a].duration == 0){
			if(cleanShop[i].typeName == "Gods Slot"){
			  ...
			  else textBtnShop += arrayShopData[a].unit+' - '+numberFormat(arrayShopData[a].token)+' Token';
		    }else textBtnShop += arrayShopData[a].unit+' - '+numberFormat(arrayShopData[a].token)+' Token';
		  }else textBtnShop += arrayShopData[a].duration+' '+arrayShopData[a].unit+' - '+numberFormat(arrayShopData[a].token)+' Token';
		  appendShopDiv += '<div class="col-xl-3 col-lg-3 col-md-6 col-sm-6 col-12">';
	      appendShopDiv += '<button id="shop-button-' + i + '-' + a + '" class="shop-button-class" value="' + arrayShopData[a].id + '"' + disableButton + '><span id ="button-desc-' + i + '-' + a + '">' + textBtnShop + '</span></button>';
	      appendShopDiv += '</div>';
		}
		appendShopDiv += '</div>';
	  }
  ...
  var showPrestigeArea = function(){
	if(hero.maxFloor >= setting.unlockReqFloorPrestige){
  ...
  var showArtifactArea = function(selectedFilterArtifact=null,selectedSortingArtifact=null){
	...
	for(let i=0; i<listArtifact.length; i++){
	  ...
	  if(multiplierArtifact > 0) artifactDamage = (artifactData.baseDamage * multiplierArtifact) + (unlockArtifact.damage*1);
	  else if(multiplierArtifact == 0){
		artifactDamage = (artifactData.baseDamage * searchArrayArtifactLevel.maxLevel) + (unlockArtifact.damage*1);
	  }
	  ...
	  if( !$( "#artifact-area-" + unlockArtifactId ).length){
		let appendArtifactDiv = '<div class="row col-xl-4 col-lg-6 col-md-6 col-sm-12 col-12 p-1">';
			...
			appendArtifactDiv += '<div class="col-xl-8 col-lg-8 col-md-8 col-sm-8 col-8 v-center-flex">';
			...
			appendArtifactDiv += '<div class="col-xl-4 col-lg-4 col-md-4 col-sm-4 col-4 v-center-flex">';
			appendArtifactDiv += '<span id="artifact-calc-damage-' + unlockArtifactId + '">' + artifactData.calcDamage + '</span>&nbsp;<span id="artifact-damage-' + unlockArtifactId + '">' + numberFormat(artifactDamage) + '</span>&nbsp;<span id="artifact-desc-damage-' + unlockArtifactId + '">' + artifactData.descDamage + '</span>';
			appendArtifactDiv += '</div>';
		...
		componentAction ("artifact-damage-"+unlockArtifactId, "replaceText", artifactDamage);
	...
	for(let i=0;i<hero.achievement.length;i++){
	  ...
	  if( !$( "#achievement-area-" + i ).length){
		let appendAchievementDiv = '<div id="achievement-area-' + i + '" class="row border-top no-gutters rounded-card">';
			...
			appendAchievementDiv += '<button id="achievement-button-' + i + '" class="upgrade-level-button mt-1 '+notifIconClass+'" value="' + i + '"' + disableButton + '><span id="achievement-reward-' + i + '">' + buttonText + '</span></button>';
			appendAchievementDiv += '</div>';
			appendAchievementDiv += '</div>';
		$("#achievement-area-div").append(appendAchievementDiv).off('click','#achievement-button-'+i).on('click','#achievement-button-'+i,function(){
		  collectAchievementReward(this); 
	    });
	  }else{
		...
		if(disableButton == '') $("#achievement-button-"+i).prop('disabled', false);
		else $("#achievement-button-"+i).prop('disabled', true);
	  }
  ...
  var checkTowerReward = function(towerId, selectedFloor){
	...
	switch(towerId){
	  ...
	  case 3:
	  case 5:
		if(towerData.priceMultiplierType == "%") currentReward = new Decimal(towerData.multiplierValue).div(100).pow(selectedFloor-1).mul(towerData.basePrice);
		else currentReward = new Decimal(towerData.multiplierValue).mul(selectedFloor-1).plus(towerData.basePrice);
		return numberFormat(currentReward);
		break;
	  case 4:
  ...
  var getTowerReward = function(towerId, towerReward){
	switch(towerId){
	  case 0:
	  ...
	  case 1:
		...
		break;
	  case 2:
	    hero.spiritStone = hero.spiritStone + (towerReward*1);
		break;
	  case 3:
	    hero.sphere = hero.sphere + (towerReward*1);
		break;
	  case 4:
	    let towerRewardArray = towerReward;
		insertNewUnlockEquipment(true,-1,towerRewardArray);
		break;
	  case 5:
	    hero.soul = hero.soul + (towerReward*1);
		break;		
	  case 6:
	    /*treasure*/
		break;
	...
	if(towerId != 4){
	  $("#modalActionTower").modal("show");
	  componentAction("tower-information-div", "replaceText", "Congratz, you managed to beat the tower manager",true);
	  componentAction("tower-information-ok-button","inputValue","success",true);
	}
	updateTask(27);
	showDetailTowerArea("tower-area-"+towerId,true);
	$("#tower-battle-area-"+towerId).remove();
  };
  
  let actionTower = function(e){
	...
	if(totalTowerDamage.lessThanOrEqualTo(0)){
	  canRunTower = false;
	  notificationAlert = "You must have at least 1 partner to get started";
	...
	if(canRunTower){
	  if(towerFloor <= unlockTower.currentFloor){
		...
		getTowerReward(towerId,currentReward);
	  }else{
	    ...
	    appendTowerActionDiv += '<div class = "col-xl-12 col-lg-12 col-md-12 col-sm-12 col-12">Prize :&nbsp;'+currentReward+'&nbsp;<img src="assets/images/'+prizeImg+'.svg" class="image-size-2"></img></div>';
	    ...
	    restTowerBossTimer = (setting.baseTowerTimeLimit + (hero.bonusTowerTimeLimit*1)) * 1000;
  ...
  var showDetailTowerArea = function(id,forceRun){
	let towerId = parseInt(id.replace('tower-area-',''),10);
	let towerData = tower.find(item => item.id === towerId);
	let unlockTower = hero.unlockTower.find(item => item.id === towerId);
	let towerName = towerData.name;
	let towerImg = towerName.toLowerCase().split(' ').join('-');
	if(unlockTower){
	  ...
	  for(let i=unlockTower.currentFloor+9;i>=0;i--){
		...
		appendTowerDetailDiv += '<div class = "col-xl-7 col-lg-7 col-md-7 col-sm-7 col-6">'+currentReward+'&nbsp;';
  ...
  var showTowerArea = function(){
	...
	for(let i=0;i<tower.length;i++){
	  ...
	  if( !$( "#tower-area-" + towerId ).length){
		...
		appendTowerDiv += '<div id="tower-area-' + towerId + '" class="row col-xl-12 col-lg-12 col-md-12 col-sm-12 col-12 m-0 cursor-button rounded-card middle-align tower-area">';
		appendTowerDiv += '<div class = "col-xl-12 col-lg-12 col-md-12 col-sm-12 col-12">';
		appendTowerDiv += '<img src="assets/images/'+towerImg+'.svg" class="medium-img">';
		appendTowerDiv += '</div>';
		...
		if(hero.maxFloor >= towerData.unlockReqFloor && towerData.unlockReqFloor > 0){
		  appendTowerDiv += '<div class = "col-xl-12 col-lg-12 col-md-12 col-sm-12 col-12">Available Key : '+unlockTower.availableKey+"/"+setting.availableKeyEachTower+'</div>';
		...
		if(hero.maxFloor < towerData.unlockReqFloor || towerData.unlockReqFloor == 0){
		  ...
		  else appendTowerDiv += '<div class="middle-align unlock-info">Unlock at Floor '+towerData.unlockReqFloor+'</div>';
		  appendTowerDiv += '</div>';
		}
		appendTowerDiv += '</div>';
		
		$("#tower-area-div").append(appendTowerDiv).off('click','.tower-area').on('click','.tower-area',function(){
		  showDetailTowerArea(this.id);
		});
	  }
	}
	calcHeightScrollDiv("tower");
  ...
  var showGodsArea = function(selectedGodsId=null,clearDiv=false){
	...
	if(hero.highestFloor >= setting.unlockReqFloorGods){
	  if(hero.unlockFragment.length > 0){
	    ...
	    for(let i=0; i<tempUnlockFragment.length; i++){
		  ...
		  if( !$( "#gods-area-" + unlockFragment.id ).length){
			...
			if(godsActiveStyle == ""){
			  componentAction ("gods-area-"+unlockFragment.id, "removeClass", "active-bg-color");
			}else componentAction ("gods-area-"+unlockFragment.id, "addClass", "active-bg-color");
	  ...
	  if( !$( "#god-unlock-div").length){
	    let appendGodsDiv = '<div id="god-unlock-div" class="rounded-card unlock-info middle-align">Unlock this gods at floor ' + setting.unlockReqFloorGods + ' </div>';
	    $("#gods-area-div").append(appendGodsDiv);
	  }
	}
	...
	if(hero.highestFloor >= setting.unlockReqFloorGods){
	  ...
	  if(!$( "#temple-area" ).length){
	    let appendTempleDiv = '<div id="temple-area" class="row border-top rounded-card no-gutters mx-1">';
		    ...
		    appendTempleDiv += '</div>';
		    appendTempleDiv += '<div class="col-xl-12 col-lg-12 col-md-12 col-sm-12 col-12 animate-bar no-padding mt-2-5-cond">';
	  ...
	  if( !$( "#temple-unlock-div").length){
	    let appendTempleDiv = '<div id="temple-unlock-div" class="rounded-card unlock-info middle-align">Unlock this temple at floor ' + setting.unlockReqFloorGods + ' </div>';
	...
	if(isInsertNewDailyTask || isInsertNewWeeklyTask || isInsertNewMonthlyTask){
	  ...
	  for(let i=1;i<dailyTask.length;i++){
		...
		switch(dailyTask[i].id){
		  ...
		  case 20:
		  case 21:
		    if(hero.highestFloor < setting.unlockReqFloorBarracks) addDailyTask = false;
			break;
		  case 22:
		  case 23:
		  case 24:
		    if(hero.highestFloor < setting.unlockReqFloorGods) addDailyTask = false;
			break;
		  case 25:
		  case 26:
		    if(hero.highestFloor < setting.unlockReqFloorBarracks) addDailyTask = false;
			break;
		  case 27:
		    if(hero.highestFloor < setting.unlockReqFloorTower) addDailyTask = false;
			break;
		  case 28:
		    if(hero.highestFloor < setting.unlockReqFloorLabor) addDailyTask = false;
			break;
		}
  ...
  var unlockNewFeatures = function(featureName=""){
	let newFeature = "";
	let notifText = "";
	let newDwellers = 0;
	...
	if(featureName != ""){
	  ...
	  if(hero.highestFloor == setting.unlockReqFloorTower) unlockNewTower();
	  switch(hero.highestFloor){
		case setting.unlockReqFloorLabor:
		  if(hero.countLabor == 0){
		    insertNewUnlockLabor();
			showNotifIcon("labor-tab-button");
		  }
		  break;
		case setting.unlockReqFloorBarracks:
	      if(hero.activeTrainingSlot == 0){
			newDwellers = 1;
		...
		case setting.unlockReqFloorEquipment:
		  ...
		  break;
	    case setting.unlockReqFloorGods:
	      if(hero.activeGodsSlot == 0){
		    newFeature = "gods";
			newDwellers = 1;
			abilityDwellersName = 'cleric';
		    hero.activeGodsSlot = 1;
		  }
		  insertAvailableGods();
		  break;
	  };
  ...
  var showDetailPartner = function(e){
	...
	if(compareDivId != partnerId){
	  ...
	  if(highestUnlockPartner){
		...
		appendDetailPartner += '<div id="partner-fragment" class="row col-xl-6 col-lg-6 col-md-6 col-sm-6 col-6 m-0 px-0 mb-1">';
		appendDetailPartner += '<img src="assets/images/partner-fragment.svg" class="image-size-2"></img>&nbsp;Partner Fragment :&nbsp;<span id="total-partner-fragment">'+numberFormat(partnerFragment)+'</span>';
		appendDetailPartner += '</div>';
		appendDetailPartner += '<div id="partner-sphere" class="row col-xl-6 col-lg-6 col-md-6 col-sm-6 col-6 m-0 px-0 mb-1">';
		appendDetailPartner += '<img src="assets/images/sphere.svg" class="image-size-2"></img>&nbsp;Sphere :&nbsp;<span id="total-partner-sphere">'+numberFormat(equipmentSphere)+'</span>';
		appendDetailPartner += '</div>';
		appendDetailPartner += '<div id="partner-blueprint" class="row col-xl-6 col-lg-6 col-md-6 col-sm-6 col-6 m-0 px-0 mb-1">';
		...
		appendDetailPartner += '<h6 id="stat-partner-tab-'+partnerId+'" class="partner-tab mx-0 rounded-card tab-button-active cursor-button font-weight-bold text-primary v-center-flex middle-content">Stat&nbsp;<img src="assets/images/info-color.svg" class="image-size-static cursor-button partner-stat-info"></img></h6>';
		appendDetailPartner += '</div>';
		...
		}).off('.click','.partner-stat-info').on('click','.partner-stat-info',function(){
		  $("#modalNotification").modal("show");
		  componentAction ("notification-text", "replaceText", "Partner level follows the total level of the base dps, ability damage, crit chance and crit damage"); 
		});
  ...
  var enhancePartner = function(partnerId){
	...
	if(highestUnlockPartner){
	  ...
	  if(hero.partnerFragment >= requiredPartnerFragment){
		...
		$("#modalNotification").modal("show");
	    componentAction ("notification-text", "replaceText", "You don't have enough partner fragment to upgrade");
	  }
	}
	saveGame();
  };
  ...
  var showDetailPartnerStat = function(partnerId){
	...
	if(highestUnlockPartner){
	  ...
	  if(canEnhance) appendPartnerStatDiv += '<div class="v-center-flex">Enhance Partner&nbsp;<img src="assets/images/partner-fragment.svg" class="image-size-static cursor-button"><span id="required-partner-fragment-'+partnerId+'">'+numberFormat(requiredPartnerFragment)+'</span></div>';
	...
	if(highestUnlockPartner){
	  ...
	  if(unlockItem){
		...
		if(unlockItem.storageType != "warehouse" && unlockItem.storageType != "backpack"){
		  ...
		  let bonusWeaponPercentage = new Decimal(formulaPartnerWeaponDamage(partnerData.id));
		...
		switch(actionType.toLowerCase()){
		  case "upgrade":
			...
			actionDesc = '<img src="assets/images/sphere.svg" class="image-size mb-05">'+numberFormat(totalSphere);
			break;
		  case "enhance":
	  ...
	  let partnerData = partner.find(item => item.id === partnerId);
	  let listEquipment = hero.unlockItem.filter(item => item.storageType === "backpack" && (item.partnerType === partnerData.type || item.partnerType === "All"));
	  ...
	  $("#equipment-stat-div").append(appendDetailEquipmentStatDiv).off('click','.equip-item-button').on('click','.equip-item-button',function(){
		...
		unequipItemPartner(this);
	  }).off('click','#action-item-button-' + partnerId).on('click','#action-item-button-' + partnerId,function(){
		confirmActionItem(this,unlockItem.storageType,actionType.toLowerCase()); 
	  });
	  checkCssSetting();
	  calcHeightScrollDiv("barracks");
	}
	  ...
	  if(highestUnlockPartner.ability.length > 0){
		...
		appendDetailPartner = '<div id="detail-partner-ability-area" class="row col-xl-12 col-lg-12 col-md-12 col-sm-12 col-12 px-0">';
		  for(let a=0;a<highestUnlockPartner.ability.length;a++){
			...
			if(showAccuracy){
			  appendDetailPartner += '<button id="acc-all-minus-'+partnerId+'-'+abilityId+'" class="down-count btn-info" title="Down" value="'+partnerId+'='+abilityId+'=acc=allMinus">All</button>';
			  appendDetailPartner += '<button id="acc-minus-'+partnerId+'-'+abilityId+'" class="down-count btn-info" title="Down" value="'+partnerId+'='+abilityId+'=acc=minus">-</button>';
			}
			appendDetailPartner += '<span id="acc-counter-'+partnerId+'-'+abilityId+'">'+unlockPartnerAbility.accuracy +'</span>%';
			if(showAccuracy){
			  appendDetailPartner += '<button id="acc-plus-'+partnerId+'-'+abilityId+'" class="up-count btn-info" title="Up" value="'+partnerId+'='+abilityId+'=acc=plus">+</button>';
			  appendDetailPartner += '<button id="acc-all-plus-'+partnerId+'-'+abilityId+'" class="up-count btn-info" title="Up" value="'+partnerId+'='+abilityId+'=acc=allPlus">All</button>';
			}
			appendDetailPartner += '</div>';
			appendDetailPartner += '<div class="col-xl-6 col-lg-6 col-md-6 col-sm-6 col-12 v-center-flex my-1">Damage : ';
			appendDetailPartner += '<button id="dam-all-minus-'+partnerId+'-'+abilityId+'" class="down-count btn-info" title="Down" value="'+partnerId+'='+abilityId+'=dam=allMinus">All</button>';
			appendDetailPartner += '<button id="dam-minus-'+partnerId+'-'+abilityId+'" class="down-count btn-info" title="Down" value="'+partnerId+'='+abilityId+'=dam=minus">-</button>';
			appendDetailPartner += '<span id="dam-counter-'+partnerId+'-'+abilityId+'">'+unlockPartnerAbility.damage +'</span>%';
			appendDetailPartner += '<button id="dam-plus-'+partnerId+'-'+abilityId+'" class="up-count btn-info" title="Up" value="'+partnerId+'='+abilityId+'=dam=plus">+</button>';
			appendDetailPartner += '<button id="dam-all-plus-'+partnerId+'-'+abilityId+'" class="up-count btn-info" title="Up" value="'+partnerId+'='+abilityId+'=dam=allPlus">All</button>';
			appendDetailPartner += '</div>';
		  ...
		  }).off('click','.down-count').on('click','.down-count',function(){
		    actionCounterValue(this); 
		  }).off('click','.up-count').on('click','.up-count',function(){
		    actionCounterValue(this);
	...
	if(hero.highestFloor >= setting.unlockReqFloorBarracks){
	  ...
	  if(appendBarracksDiv  != ""){
		...
		if( !$( "#barracks-info" ).length){
		  ...
		  infoBarracksDiv += 'Please click the partner to see the detail.';
	      infoBarracksDiv += '</div>';
	    }  
		if(hero.settingOrderPartner == "asc"){
	  ...
	  if( !$("#ability-unlock-div").length){	
	    let unlockInformation = "Unlock this barracks at floor "+setting.unlockReqFloorBarracks;
  ...
  var showTrainingArea = function(){
	...
	if(hero.highestFloor >= setting.unlockReqFloorBarracks){
	  if( !$("#training-area").length){
	    ...
	    let appendTrainingDiv = '<div id="training-area" class="row border-top rounded-card">';
		    ...
		    for(let i=0;i<hero.activeTrainingSlot;i++){
			  ...
			  let trainingDesc = '';
			  let buttonText = 'Click To Choose Partner';
			  ...
			  if(trainingZone){
				...
				trainingDesc = "On progress training "+partnerAbilityData.name+" ability for "+partnerData.name+" partner";
	  ...
	  if( !$("#training-unlock-div").length){	
	    let unlockInformation = "Unlock this training ground at floor "+setting.unlockReqFloorBarracks;
  ...
  var calcRestTimeTraining = function(maxLevel){
	return Math.ceil(setting.baseTrainingAbilityHour * 3600 * Math.ceil(maxLevel/100));
  };
  
  var startTrainingTimer = function(){
	let onProgressTrainingSlot = hero.trainingZone.trainingSlot.length;
	for(let i=0;i<onProgressTrainingSlot;i++){
	  ...
	  if(restTrainingTime > 0){
	    ...
	    unlockAbilityPartner.trainingTime = (restTrainingTime * -1);
	    if(hero.settingAutoAbilityTraining){
	      trainingZone.startTraining = Date.now();
	    }else stopTrainingAbility(partnerId+"="+abilityId+"="+trainingSlot,true);
	  }
	}
  };
  ...
  var showCompanionArea = function(){
	...
	if(hero.highestFloor >= setting.unlockReqFloorCompanion){ 	
	  ...
	  for(let i=0; i<hero.countCompanion; i++){
		...
		if( !$( "#companion-area-" + unlockCompanionId ).length){
		  let appendCompanionDiv = '<div id="companion-area-' + unlockCompanionId + '" class="row border-top rounded-card ' + companionActiveStyle + '">';
			...
			appendCompanionDiv += '<div class="w-100">Damage&nbsp;: DMG +&nbsp;<span id="companion-damage-' + unlockCompanionId + '">' + numberFormat(unlockCompanion.damage) + '</span>%</div>';
			appendCompanionDiv += '<div class="w-100">Passive&nbsp;: DMG +&nbsp;<span id="companion-passive-damage-' + unlockCompanionId + '">' + numberFormat(passiveDamage) + '</span>%</div>';
			appendCompanionDiv += '</div>';
	  ...
	  if( !$( "#companion-unlock-div").length){
	    let appendCompanionDiv = '<div id="companion-unlock-div" class="rounded-card unlock-info middle-align">Unlock this companion at floor ' + setting.unlockReqFloorCompanion + ' </div>';
  ...
  var compareItemWithEquip = function(e,storageType){
	if(e.value != compareDivId){
	  ...
	  for(let i=0;i<filteredUnlockPartner.length;i++){
		...
		appendItemComparation += '<div class="w-100">Partner : ' + partnerData.name + '</div>';
  ...
  var confirmUseItem = function(e,storageType){
	...
	if(hero.settingConfirmUseItem && canUseItem){
	  componentAction("use-item-text","replaceText",unlockItem.name);
  ...
  var equipItemToPartner = function(e){
	...
	if(!$("#warehouse-div").hasClass("d-none")) updateWarehouseArea();
	  ...
	  showDetailPartnerEquipment(partnerId);
	  $("#icon-image-"+itemType.toLowerCase()+"-"+partnerId).attr("src", "assets/images/"+unlockItem.name.split(" ").join("-")+".svg");
  ...
  var unequipItemPartner = function(e){
	let arrayString = e.value.split("=");
	let partnerId = parseInt(arrayString[0],10);
	let itemType = arrayString[1];
	let itemId = parseInt(arrayString[2],10);
	...
	if(unlockItem){
	  ...
	  switch(actionType.toLowerCase()){
		case "sell":
		  ...
		  actionMaterial = Math.ceil(totalSphere/5);
		  if(actionMaterial < 1) actionMaterial = 1;
		  actionText = "Gain : "+actionMaterial+" "+actionMaterialName;
		  actionHeaderText = 'Are you sure want to sell '+equipmentData.name+' and you will '+actionText.replace(":","").toLowerCase()+' ?';
		  break;
		case "upgrade":
		  actionMaterialName = "Sphere";
	  ...
	  let actionDiv = '<div class="w-100 '+actionClass+'">'+actionText+'</div>';
	  if(canProcess){
	    let partnerImg = equipmentData.partnerType.toLowerCase();
	    ...
	    appendModalDiv += '<div class="col-xl-12 col-lg-12 col-md-12 col-sm-12 col-12 mb-2 middle-align">'+actionHeaderText+'</div>';
	    ...
	    $("#modal-action-desc").append(appendModalDiv);
  ...
  var showEquipmentList = function(unlockItem, storageType="", actionDiv="",nextMinBenefit="",nextMaxBenefit="", showInModal=false){
	...
	let appendItemListDiv = '<div class="col-xl-3 col-lg-4 col-md-4 col-sm-3 col-4 '+classImage+' '+classImgMargin+'">';
	...
	appendItemListDiv += '<img id="'+storageType+'-image-' + unlockItem.id + '" src="assets/images/'+showEquipmentIcon(equipmentData.name,true)+'" class="image-equipment right-align"></img>';
	appendItemListDiv += '</div>';
	if(enhanceLevel > 0){
	...
	appendItemListDiv += '</div>';
	appendItemListDiv += '<div class="col-xl-9 col-lg-8 col-md-8 col-sm-9 col-8 px-0">';
	appendItemListDiv += '<div class="w-100 v-center-flex">';
	appendItemListDiv += '<img src="assets/images/'+partnerImg+'.svg" class="image-size" title="'+equipmentData.partnerType+'"></img>&nbsp;';
	appendItemListDiv += '<span id="'+storageType+'-name-' + unlockItem.id + '">' + equipmentData.name + '</span>';
	appendItemListDiv += '</div>';
	appendItemListDiv += '<div class="w-100">'+benefitName+'&nbsp;:&nbsp;<span id="'+storageType+'-min-value-' + unlockItem.id + '">' + numberFormat(minValue) + '</span>'+nextMinBenefit+' - <span id="'+storageType+'-max-value-' + unlockItem.id + '">' + numberFormat(maxValue) + '</span>'+nextMaxBenefit+'</div>';
  ...
  var showItemList = function(unlockItem, storageType=""){
	...
	let appendItemListDiv = '<div class="col-xl-3 col-lg-4 col-md-4 col-sm-3 col-4 my-2 cursor-button">';
	...
	appendItemListDiv += '</div>';
	appendItemListDiv += '<div class="w-100 right-align">';
	appendItemListDiv += '<img id="'+storageType+'-image-' + unlockItem.id + '" src="assets/images/'+showEquipmentIcon(itemData.name,true)+'" class="image-equipment right-align"></img>';
	appendItemListDiv += '</div>';
	appendItemListDiv += '</div>';
	appendItemListDiv += '<div class="col-xl-9 col-lg-8 col-md-8 col-sm-9 col-8 px-0">';
	appendItemListDiv += '<div class="w-100 v-center-flex">';
  ...
  var showWarehouseArea = function(storageType="warehouse",selectedFilterItem=null,selectedSortingItem=null){
	showWarehouseTab(storageType);
	if(hero.highestFloor >= setting.unlockReqFloorWarehouse){
	  ...
	  for(let i=0;i<listStorage.length;i++){
		...
		switch(listStorage[i].itemType.toLowerCase()){
		  case "weapon":
		    ...
		    sellSphere = Math.floor(totalSphere/2);
	        if(sellSphere < 1) sellSphere = 1;
		...
		if( !$( "#"+storageType+"-item-area-" + listStorage[i].id ).length){
		  ...
		  appendStorageDiv += '<div class="col-xl-6 col-lg-6 col-md-6 col-sm-6 col-6 px-0">';
		  if(useOrEquip != "")appendStorageDiv += '<button id="equip-item-button-' + listStorage[i].id + '" class="upgrade-level-button '+useOrEquipClass+'" value="'+listStorage[i].id+'" '+useOrEquipDisable+'>'+useOrEquip+'</button>';
		  ...
		  appendStorageDiv += '</div>';
		  if(isEquipmentList){
		    appendStorageDiv += '<div class="col-xl-6 col-lg-6 col-md-6 col-sm-6 col-6 mt-1 p-0 v-center-flex">';
		    appendStorageDiv += '<button id="sell-item-button-' + listStorage[i].id + '" class="upgrade-level-button" value="'+listStorage[i].id+'">Sell <img src="assets/images/sphere.svg" class="image-size mb-05">'+numberFormat(sellSphere)+'</button>';
		    appendStorageDiv += '</div>';
		  ...
		  $("#"+storageType+"-area-div").append(appendStorageDiv).off('click','#equip-item-button-'+ listStorage[i].id).on('click','#equip-item-button-'+ listStorage[i].id,function(){
		    if(isEquipmentList) compareItemWithEquip(this,storageType); 
			  ...
			  if(hero.settingConfirmUseItem) confirmUseItem(this,storageType);
		    ...
		    confirmActionItem(this,storageType,"sell"); 
		    ...
		    confirmActionItem(this,storageType,actionType.toLowerCase()); 
	  ...
	  if(!$( "#"+storageType+"-unlock-div").length){
		...
		appendStorageDiv += '<div id="'+storageType+'-unlock-div" class="rounded-card unlock-info middle-align col-xl-12 col-lg-12 col-md-12 col-sm-12 col-12 m-0">Unlock this '+storageType+' at floor ' + setting.unlockReqFloorEquipment + ' </div>';
		appendStorageDiv += '</div>';
	...
	for(let i = 0; i<pet.length; i++){
	  ...
	  if( !$( "#pet-area-" + i ).length){
		if(unlockPet){
		  ...
		  unlockReqText = 'Unlock this pet at floor '+petData.unlockReqFloor;
		  ...
		  switch(i){
			...
			default:
			  if(i >= 3){
				...
				if(hero.highestFloor < petData.unlockReqFloor){
					unlockReqText += ' and you have unlock partner ' + partnerData.name + ' once';
					nextUnlockPet++;
			    }else{
				  unlockReqText = 'Unlock this pet if you have unlock partner ' + partnerData.name + ' once';
				}
			  }else{
  ...
  var showAdventureArea = function(selectedPetId=-1){
	...
	for(let i=0;i<hero.countZone;i++){
	  ...
	  let adventureTime = zoneData.adventureTime * 3600 * (100 - hero.bonusAdventureTimePercentage) / 100;
	  let noteButtonAdventure = '';
	  let disabledButton = '';
	...
	if(hero.countZone < zone.length){
	  ...
	  if(hero.countZone > 0){
		...
		let petData = pet.find(item => item.id === 0);  
		unlockInformation = 'Unlock this area at floor '+petData.unlockReqFloor;
	  }
  ...
  var showDwellersDesc = function(dwellersId){
	...
	switch(unlockDwellers.currentAbility.toLowerCase()){
	  case "trainer":
	    roleDesc = "Training partner ability in training ground";
		break;
	  case "warlords":
	...
	if(hero.highestFloor >= setting.unlockReqFloorHouse){
	  ...
	  if( !$( "#house-unlock-div").length){
	    let appendHouseDiv = '<div class="row col-xl-12 col-lg-12 col-md-12 col-sm-12 col-12 p-1">';
			appendHouseDiv += '<div id="house-unlock-div" class="rounded-card unlock-info middle-align col-xl-12 col-lg-12 col-md-12 col-sm-12 col-12 m-0">Unlock this house at floor ' + setting.unlockReqFloorHouse + ' </div>';
			appendHouseDiv += '</div>';
  ...
  var showBankArea = function(){
	...
	if(hero.highestFloor >= setting.unlockReqFloorHouse){
	  ...
	  if( !$( "#bank-unlock-div").length){
	    let appendHouseDiv = '<div id="bank-unlock-div" class="rounded-card unlock-info middle-align">Unlock this bank at floor ' + setting.unlockReqFloorHouse + ' </div>';
  ...
  var showTutorialMenu = function(menuName){
   ...
   var showSettingArea = function(){
	...
	if(hero.settingConfirmUseItem){
	  $("#setting-confirm-use-item-true").prop('checked', true);
	  $("#setting-confirm-use-item-false").prop('checked', false);
	}else{
	  $("#setting-confirm-use-item-true").prop('checked', false);
	  $("#setting-confirm-use-item-false").prop('checked', true);
	}
  ...
  var calcDefaultModalWidth = function(){
  ...
  var calcHeightScrollDiv = function(headerName,currentActiveMenu="",cssType="max-height"){
	...
	if(headerName == "modal") $('.modal-scrollable').css({"max-height":scrollHeight});
	  ...
	  if(cssType=="max-height")$('.pre-scrollable').css({"max-height":scrollHeight});
	  else $('.pre-scrollable').css({"min-height":scrollHeight});
	}
	checkCssSetting();
  };
  ...
  var loopTowerAction = function(showAnimation,newTowerDpsInterval){
	...
	let maxTowerLimit = (setting.baseTowerTimeLimit + (hero.bonusTowerTimeLimit*1)) * 1000;
	if(showAnimation){
  ...
  var loopAction = function(showAnimation=false, newDpsInterval){
	...
	if(dpsToCoinTimer > 0){
	  ...
	  if(dpsToCoinInterval.greaterThanOrEqualTo(dpsToCoinTimer)){
	    runDpsToCoin(totalDps.plus(weaponDamage));
	...
	let totalDamage = new Decimal(totalDps).plus(partnerAbilityDamage).plus(weaponDamage);
  ...
  var idleLoopAction = function(idleLoop, newDpsInterval, callback){
	idleLoop *= 1;
	let totalDps = new Decimal(calcTotalDps());
	let maxHpEnemy = new Decimal(enemy.maxHp);
	...
	while(currentEnemyHP.greaterThan(0)){
	  ...
	  currentEnemyHP = currentEnemyHP.sub(totalDps).sub(weaponDamage);
	  killedHit += 1;
  ...
  var formulaIntervalDps = function(){
	let newDpsInterval = 1000 - (hero.bonusIntervalDps*1); 
	return newDpsInterval;
  };
  
  var gameLoop = function(showAnimation=false,callback){
	loopsEverySec++;
	if(hero.highestFloor >= setting.unlockReqFloorCompanion && hero.filledCompanionSlot < hero.generatedCompanionSlot) updateCompanionTimer();
	if(hero.highestFloor >= setting.unlockReqFloorEquipment	&& hero.filledEquipmentSlot < hero.generatedEquipmentSlot) updateEquipmentTimer();
	...
	if(hero.highestFloor >= setting.unlockReqFloorTower && loopsEverySec % 60 == 0) addTowerKey();
	if(hero.highestFloor >= setting.unlockReqFloorLabor && loopsEverySec % 10 == 0) produceLaborCoin();
  ...
  var startAutoClickerInterval = function (skillId,useTicket=false){
	...
	let intervalSkill = new Decimal(1000).div(totalSkillDamage).floor();
  ...
  var startPowerShotInterval = function (skillId,useTicket=true) {
	...
	if(!useTicket && unlockSkill.isActive) useTicket = unlockSkill.useTicket;
	if(powerShotDuration){ 
	  powerShotDuration = clearInterval(powerShotDuration);
	  ...
	  powerShotDuration = setInterval (function (){ 
	    restDurationSkill = totalDurationSkill.sub(Math.floor((Date.now() - unlockSkill.lastStartDatetime) / 1000));
		componentAction ("skill-timer-desc-"+skillId, "replaceText", "Click to Stop - "+offlineTimeText(restDurationSkill));
  ...
  var startSpecialForceInterval = function (skillId,useTicket=true) {
	...
	if(!useTicket && unlockSkill.isActive) useTicket = unlockSkill.useTicket;
	if(specialForceDuration){ 
	  specialForceDuration = clearInterval(specialForceDuration);
	  ...
	  specialForceDuration = setInterval (function (){ 
	    restDurationSkill = totalDurationSkill.sub(Math.floor((Date.now() - unlockSkill.lastStartDatetime) / 1000));
		componentAction ("skill-timer-desc-"+skillId, "replaceText", "Click to Stop - "+offlineTimeText(restDurationSkill));
  ...
  var startLuckyCoinInterval = function (skillId,useTicket=true) {
	...
	if(!useTicket && unlockSkill.isActive) useTicket = unlockSkill.useTicket;
	if(luckyCoinDuration){ 
	  luckyCoinDuration = clearInterval(luckyCoinDuration);
	  ...
	  luckyCoinDuration = setInterval (function (){ 
		restDurationSkill = totalDurationSkill.sub(Math.floor((Date.now() - unlockSkill.lastStartDatetime) / 1000));
		componentAction ("skill-timer-desc-"+skillId, "replaceText", "Click to Stop - "+offlineTimeText(restDurationSkill));
		if(restDurationSkill.lessThanOrEqualTo(0)){
		  luckyCoinDuration = clearInterval(luckyCoinDuration);
  ...
  var loopBillionaire = function(){
	...
	let newBillionaireInterval = new Decimal(1000);
  ...
  var loopGrimReaper = function(){
	let skillId = 5;
	let unlockSkill = hero.unlockSkill.find(item => item.id === skillId);
	let totalDurationSkill = new Decimal(unlockSkill.duration).plus(artifactSkillDuration(skillId).mul(1000));
	let endSkillDateTime = unlockSkill.lastStartDatetime + (totalDurationSkill*1000);
	let newGrimReaperInterval = new Decimal(5000).sub(hero.bonusIntervalGrimReaper);
	let interval = Date.now() - lastGrimReaperHit;
	while(interval >= newGrimReaperInterval){
  ...
  var startCooldownSkill = function(skillId) {
	let unlockSkill = hero.unlockSkill.find(item => item.id === skillId);
	let totalCooldownSkill = new Decimal(unlockSkill.cooldown).sub(artifactSkillCooldown(skillId));
	let cooldownTimer = 0;
  ...
  var upgradeLevelPartner = function(e){
	if(e){
      ...
      if(heroCoin.greaterThanOrEqualTo(upgradeFee)){
		...
		if(unlockPartner){
		  let highestUnlockPartner = hero.highestUnlockPartner.find(item => item.id === partnerId);
		  unlockPartner.status = highestUnlockPartner.status;
		  let unlockPartnerDps = new Decimal(unlockPartner.dps);
		  
		  if(setting.levelUpgradeMultiplier > 0){
			if(unlockPartner.level > 0)	unlockPartner.level += setting.levelUpgradeMultiplier;
  ...
  var upgradeLevelLabor = function(e){
	...
	else upgradeFee = unlockLabor.baseFee;
      ...
      if(heroOrbs.greaterThanOrEqualTo(upgradeFee)){
		...
		if(unlockArtifact){
		  if(unlockArtifact.level < artifactData.maxLevel || artifactData.maxLevel == 0){
		    ...
		    hero.artifactDamagePercentage = artifactDamage.plus(artifactData.tier*5);
  ...
  var permanentShopCalculation = function(typeName){
	let bonusMultiplier = new Decimal(0);
	switch(typeName){
	  case 'DMG':
		bonusMultiplier = new Decimal(hero.bonusShopDpcPercentage).div(500).floor().plus(1);
	    hero.bonusShopDpcPercentage = new Decimal(100).mul(bonusMultiplier).plus(hero.bonusShopDpcPercentage);
	    break;
	  case 'DPS':
	    bonusMultiplier = new Decimal(hero.bonusShopDpsPercentage).div(500).floor().plus(1);
	    hero.bonusShopDpsPercentage = new Decimal(100).mul(bonusMultiplier).plus(hero.bonusShopDpsPercentage);
		break;
	  case 'Coin':
	    bonusMultiplier = new Decimal(hero.bonusShopCoinPercentage).div(500).floor().plus(1);
	    hero.bonusShopCoinPercentage = new Decimal(100).mul(bonusMultiplier).plus(hero.bonusShopCoinPercentage);
		break;
	};		
  };
	  ...
	  switch(shopData.typeName){
		case 'DMG':
		  if(bonusDuration == 0){
			...
			else permanentShopCalculation(shopData.typeName);
		  }else{
		...
		case 'All Boost':
		  if(bonusDuration == 0){
			if(new Decimal(hero.bonusShopDpcPercentage).equals(0)) hero.bonusShopDpcPercentage = new Decimal(100);
			else permanentShopCalculation("DMG");
			if(new Decimal(hero.bonusShopDpsPercentage).equals(0)) hero.bonusShopDpsPercentage = new Decimal(100);
			else permanentShopCalculation("DPS");
			if(new Decimal(hero.bonusShopCoinPercentage).equals(0)) hero.bonusShopCoinPercentage = new Decimal(100);
			else permanentShopCalculation("Coin");
		  }else{
			if(hero.tempBonusShopDpcTime == 0) hero.tempBonusShopDpcTime = endDatetime;
			else hero.tempBonusShopDpcTime += bonusDuration;
		...
		case 'Gods Slot':
		  ...
		  break;
		case 'Orbs':
		  if(bonusDuration == 0) hero.bonusShopOrbsPercentage = new Decimal(hero.bonusShopOrbsPercentage).plus(100);
		  else successShop = false; 
		  break;
		case 'Ticket':
		case 'Instant Coin':
		  ...
		  if(materialArray.length > 0){
			for(let i=0;i<materialArray.length;i++){
			  let materialId = materialArray[i];
			  let unlockItem = hero.unlockItem.find(item => item.itemId === materialId);
  ...
  var calcSellArtifact = function(artifactId){
	...
	let upgradeFee = new Decimal(calcCurrentUpgradeArtifactFee(artifactId)).mul(100-hero.bonusArtifactUpgradeFeePercentage).div(100).ceil();
  ...
  var sellArtifact = function(artifactId){
	...
	if (index !== -1){
	  ...
	  switch (artifactId){
		case 0:
		  hero.bonusCoinPercentage = 0;
		  break;
		case 1:
		  hero.bonusBossCoinPercentage = 0;
		  break;
		case 2:
		case 3:
		  hero.bonusDpcPercentage = 0;
		  break;
		case 4:
		  hero.bonusDpsPercentage = 0;
		  break;
		case 5:
		  hero.bonusOfflineCoinPercentage = 0;
		  break;
		case 6:
		  hero.bonusOfflineDpsPercentage = 0;
		  break;
		case 7:
		  artifactDamage = artifactDamage.sub(totalDamage);
		  break;
		case 8:
		  hero.bonusOrbsPercentage = 0;
		  break;
		case 9:
		  hero.bonusOfflineTimeLimit = 0;
		  break;
		case 10:
		  hero.bonusIntervalDps = 0;
		  startDpsInterval(true);
		  break;
		case 11:
		  hero.bonusIntervalGrimReaper = 0;
		  break;
		case 27:
		  hero.bonusBossTimeLimit = 0;
		  break;
		case 28:
		  hero.bonusEnemyHpPercentage = 0;
		  break;
		case 29:
		  hero.bonusBossHpPercentage = 0;
		  break;
		case 30:
		  hero.bonusHeroUpgradeFeePercentage = 0;
		  break;
		case 31:
		  hero.bonusWeaponUpgradeFeePercentage = 0;
		  break;
		case 32:
		  hero.bonusPartnerUpgradeFeePercentage = 0;
		  break;
		case 33:
		  hero.bonusSkillUpgradeFeePercentage = 0;
		  break;
		case 34:
		  hero.bonusChanceSkipFloorPercentage = 0;
		  break;
		case 38:
		  hero.bonusAdventureExpPercentage = 0;
		  break;
		case 39:
		  hero.bonusAdventureCoinPercentage = 0;
		  break;
		case 40:
		  hero.bonusAdventureTimePercentage = 0;
		  break;
		case 41:
		  hero.bonusDoubleAdventurePercentage = 0;
		  break;
		case 42:
		  hero.bonusArtifactRerollFeePercentage = 0;
		  break;
		case 43:
		  hero.bonusArtifactUnlockFeePercentage = 0;
		  break;
		case 44:
		  hero.bonusArtifactUpgradeFeePercentage = 0;
		  break;
		case 45:
		  hero.bonusMeleeDpsPercentage = 0;
		  break;
		case 46:
		  hero.bonusStealthDpsPercentage = 0;
		  break;
		case 47:
		  hero.bonusSwordDpsPercentage = 0;
		  break;
		case 48:
		  hero.bonusRangedDpsPercentage = 0;
		  break;
		case 49:
		  hero.bonusMagicDpsPercentage = 0;
		  break;
		case 50:
		  hero.bonusFirearmDpsPercentage = 0;
		  break;
		case 51:
		  hero.bonusRuneDpsPercentage = 0;
		  break;  
		case 52:
		  hero.bonusAllDamagePercentage = 0;
		  break;
		case 53:
		  hero.bonusAllCoinPercentage = 0;
		  break;
		case 54:
		  hero.bonusAbilityPercentage = 0;
		  break;
		case 55:
		  hero.bonusMeleeAbilityPercentage = 0;
		  break;
		case 56:
		  hero.bonusStealthAbilityPercentage = 0;
		  break;
		case 57:
		  hero.bonusSwordAbilityPercentage = 0;
		  break;
		case 58:
		  hero.bonusRangedAbilityPercentage = 0;
		  break;
		case 59:
		  hero.bonusMagicAbilityPercentage = 0;
		  break;
		case 60:
		  hero.bonusFirearmAbilityPercentage = 0;
		  break;
		case 61:
		  hero.bonusRuneAbilityPercentage = 0;
		  break;
		case 62:
		  hero.bonusCritChancePercentage = 0;
		  break;
		case 63:
		  hero.bonusCritDamagePercentage = 0;
		  break;
	  }
	  calculateAllArtifactDamage();
	...
	if(highestUnlockPartner){
	  ...
	  if(unlockAbility){
		...
		if(!unlockAbility.isActive){
		  ...
		  unlockPartner.abilityId = abilityId;
		  calculateAllPartnerAbility();
  ...
  var chooseTrainingPartner = function(e){
	...
	if(trainingSlot){
	  ...
	  let dropDownList = ["-1=-- Please Choose Partner --"];
  ...
  var stopTrainingAbility = function(e,completeTraining=false){
	...
	if(trainingSlot){
	  ...
	  let trainingTime = unlockAbilityPartner.trainingTime + Math.floor((Date.now() - startTraining)/1000);
  ...
  var startAdventurePet = function(arrayString){
	...
	if(unlockZone){
	  ...
	  if(!unlockZoneSlot){
	    ...
	    let endAdventureTime = startAdventureTime + (zoneData.adventureTime * 3600 * 1000 * (100 - hero.bonusAdventureTimePercentage) / 100);
	  ...
	  if(!objAdventureInterval){
	    adventureInterval = setTimeout(function loopCooldownAdventureTimer() {
	      ...
	      if(cooldownTimer <= 0) stopCooldownAdventureTimer(petId, zoneId, slotId, afterOffline);
			...
			adventureInterval = setTimeout(loopCooldownAdventureTimer, 1000);
	  ...
	  for(let a=0;a<hero.unlockZone[i].currSlot;a++){
		...
		if(activeAdventureSlot){
		  ...
		  if(activeAdventureSlot.statusAdventure != "cancel"){
			let endAdventureTime = activeAdventureSlot.startAdventure + (zoneData.adventureTime * 3600 * 1000);
  ...
  var checkAdventureStatus = function(afterOffline=false){
	...
	if(countCompleteAdventure > 0 && !afterOffline){
	  if(countCompleteAdventure == 1){
		...
		if(($("#modalRewardAdventureConfirm").data('bs.modal') || {})._isShown ) $("#modalRewardAdventureConfirm").modal("toggle");
  ...
  var collectAchievementReward = function(e){
	let achievementId = parseInt(e.value,10);
  ...
  var insertNewUnlockLabor = function(){
	...
	let prevLaborCost = new Decimal(0);
	let prevLaborPrize = new Decimal(0);
	let maxLabor = 0;
	
	if(unlockLaborCount == 0){
	  prevLaborCost = new Decimal(setting.baseLaborCost);
	  ...
	  if(lastUnlockLabor.level > 0){
	    prevLaborCost = new Decimal(lastUnlockLabor.baseFee);
	...
	for(let i=0;i<maxLabor;i++){
	  ...
	  let rowStyle = "";
	  let baseLaborCostLevel = new Decimal(0);
	  let baseLaborPrizeLevel = new Decimal(0);
	  if(index == 0){
		baseLaborCostLevel = prevLaborCost;
	    baseLaborPrizeLevel = prevLaborPrize;    
	  }else{
		baseLaborCostLevel = prevLaborCost.mul(setting.laborCostMultiplier * index);
	    baseLaborPrizeLevel = prevLaborPrize.mul(setting.laborCostMultiplier * index).mul(setting.laborCostPrizeMultiplier).round();  
	  }
	  prevLaborCost = baseLaborCostLevel;
	  prevLaborPrize = baseLaborPrizeLevel;

	  let newObjLabor = {
		id: index,
		level: 0,
		baseFee: baseLaborCostLevel,
	...
	if(dropEquipmentGradeId < 0){
	  if(hero.countBackpack < hero.maxBackpack || fromCheckIn == true){
	    ...
	    if(unlockGradeEquipment.length > 0){
		  ...
		  let arrayDropEquipmentGrade = filterArray(dropRateEquipment,dropPercentage,1);
  ...
  var writeDungeonBattleLog = function(logText,recentCoin){
	...
	if(new Decimal(recentCoin).greaterThan(0)){
	  ...
	  $("#recent-coin").text("+"+numberFormat(recentCoin));
  ...
  var calcTotalDpc = function (isManualClick=false) {
	let skillId = 1;
	let unlockSkill = hero.unlockSkill.find(item => item.id === skillId); 
	let baseDpc = calcBaseDpc().plus(calcCompanionDpc());
	let subTotalPercentage = new Decimal(100).plus(hero.bonusDpcPercentage).plus(hero.bonusAllDamagePercentage).plus(hero.artifactDamagePercentage).plus(hero.companionAllDamagePercentage);
	let totalPercentage;
	...
	if(unlockSkill && unlockSkill.isActive){
	  ...
	  totalDpc = totalDpc.mul(totalDamagePercentage).round();
	}
	
	let dpsToDpcDamage = new Decimal(hero.companionDpsToDpcPercentage).plus(hero.enhanceDpsToDpcPercentage).div(100).mul(calcTotalDps());
  ...
  var calcBattlingDps = function(battleType,isLoopAction=false){
	...
	for(let i=0;i<battlingPartner.length;i++){
	  ...
	  else totalDps = totalDps.plus(calcTotalPartnerDps(battlingPartner[i].id,battleType));
	}
	if(isLoopAction) hero.dps = totalDps.floor();
	return totalDps;
  };
  
  var calcTotalDps = function (isOnline=true,battleType="dungeon",isLoopAction=false) {
	let baseDps = new Decimal(calcBattlingDps(battleType,isLoopAction));
	let subTotalPercentage = new Decimal(100).plus(hero.bonusAllDamagePercentage).plus(hero.artifactDamagePercentage).plus(hero.companionAllDamagePercentage).plus(hero.companionAllDpsPercentage);
	...
	let totalDps = new Decimal(baseDps).mul(totalPercentage);
	
	if(isOnline){
	  let percentageBonusDps = new Decimal(hero.percentageBonusDps);
	  ...
	  if(unlockSkill && unlockSkill.isActive){
	    let totalDamagePercentage = new Decimal(100).plus(unlockSkill.damage).plus(artifactSkillDamage(skillId)).div(100);
	    totalDps = totalDps.mul(totalDamagePercentage).round();
	  }
	}
	...
	totalDps = increaseGodsDps(totalDps);
	...
	if(battleType == "dungeon"){
	  if(hero.floor % enemy.bossFloor == 0 && enhanceDpsForBossEnemyPercentage.greaterThan(0)){
	    enhanceBonusPercentage = enhanceBonusPercentage.plus(enhanceDpsForBossEnemyPercentage);	
	  }else if(hero.floor % enemy.bossFloor != 0 && enhanceDpsForNormalEnemyPercentage.greaterThan(0)){
	...
	if(battleType == "tower") totalDps = new Decimal(100).plus(hero.bonusTowerDamage).div(100).mul(totalDps);
	if(isLoopAction) hero.totalDps = totalDps.round();
	return hero.totalDps;
  };
  ...
  var formulaRewardCoin = function(floorNumber){
	let rewardCoin = new Decimal(floorNumber).div(1.2).mul(enemy.baseCoin);
	return rewardCoin;
  };
  
  var calcTotalRewardCoin = function (isOnline=true,idleRun=false) {
	let rewardCoin = formulaRewardCoin(hero.floor);
	let subTotalPercentage = new Decimal(100).plus(hero.bonusAllCoinPercentage).plus(hero.artifactDamagePercentage).plus(hero.companionAllCoinPercentage);
	...
	if(hero.floor % enemy.bossFloor == 0){
	  let bossCoinPercentage = new Decimal(hero.bonusBossCoinPercentage).plus(hero.companionBossCoinPercentage).plus(hero.enhanceCoinForBossEnemyPercentage).plus(hero.enhanceCoinForAllEnemyPercentage).plus(100).div(100);
  ...
  var calcTotalOrbs = function (){
	let baseOrbs = new Decimal(((hero.maxFloor - setting.unlockReqFloorPrestige)/setting.prestigeFloorBonus)+(setting.baseGainOrbsCalculation)).pow(setting.orbsMultiplier);
	baseOrbs = baseOrbs.mul(1+((hero.maxFloor-setting.unlockReqFloorPrestige)/10));
	let partnerOrbs = new Decimal(calcTotalPartnerLevel()).plus(hero.heroLevel).plus(hero.weaponLevel).div(setting.prestigePartnerLevelBonus);
  ...
  var calcTotalAbility = function(idleLoop=1){
	...
	for(let i=0;i<battlingUnlockPartner.length;i++){
	  ...
	  if(idleLoop == 1){
		unlockPartner.abilityCounter += 1;
  ...
  var calcTotalWeaponDamage = function(){
	let battlingPartner = hero.unlockPartner.filter(item => item.status.toLowerCase() === "battling");
	...
	if(battlingPartner.length > 0 && equipmentMinValue.greaterThan(0) && equipmentMaxValue.greaterThan(0)){
	  let totalWeaponDamage = new Decimal(10).pow(hero.equipmentExponentValue).mul(randomBetweenNumber(equipmentMinValue,equipmentMaxValue));
  ...
  var calcCurrentUpgradeArtifactFee = function(artifactId){
	...
	if(upgradeArtifact){
	  for(let a=1; a<upgradeArtifact.level; a++){
	 	lastUpgradeFee = new Decimal(setting.upgradeArtifactBaseTier + (artifactData.tier*setting.upgradeArtifactTierMultiplier)).mul(Math.pow((setting.upgradeArtifactMultiplier*artifactData.upgradeMultiplier),(a+1))).ceil();
	  ...
	  if(multiplierArtifact > 0){
	    for(let i=0;i<multiplierArtifact;i++){
		  if((upgradeArtifact.level+i+1 <= artifactData.maxLevel && artifactData.maxLevel > 0) || artifactData.maxLevel == 0){
	        let currentFee = new Decimal(setting.upgradeArtifactBaseTier + (artifactData.tier*setting.upgradeArtifactTierMultiplier)).mul(Math.pow((setting.upgradeArtifactMultiplier*artifactData.upgradeMultiplier),(upgradeArtifact.level+1+i))).ceil();
		...
		let canLoop = true;
	    do{
		  let currentFee = new Decimal(setting.upgradeArtifactBaseTier + (artifactData.tier*setting.upgradeArtifactTierMultiplier)).mul(Math.pow((setting.upgradeArtifactMultiplier*artifactData.upgradeMultiplier),(upgradeArtifact.level+1+a))).ceil();
		  currentFee = currentFee.mul(100-hero.bonusArtifactUpgradeFeePercentage).div(100).ceil();
  ...
  var calcOfflineTime = function () {
	...
	if(offlineTime.greaterThanOrEqualTo(0)){
	  let offlineDps = new Decimal(calcTotalDps(false));

	  let totalOfflineTimeLimit = new Decimal(setting.baseOfflineTimeLimit).plus(hero.bonusOfflineTimeLimit).mul(60);
	  let offlineTimeReward = offlineTime;
	  
	  if(offlineTime.greaterThan(totalOfflineTimeLimit)) offlineTimeReward = totalOfflineTimeLimit;
	  if(offlineDps.greaterThan(0) && offlineTimeReward.greaterThanOrEqualTo(3)){
		...
		let maxBossTimer = formulaBossTimer();
		let dpsInterval = formulaIntervalDps();

		if(hero.settingOfflineProgress){
		  let canProgress = true;
		  while(offlineTimeReward.greaterThan(0) && canProgress){
			enemyHP = initEnemy();
			rewardCoin = new Decimal(calcTotalRewardCoin(false));
			killingTime = new Decimal(enemyHP).div(offlineDps).ceil().mul(formulaIntervalDps()).div(1000);

			if(hero.floor % enemy.bossFloor == 0){
			  if((killingTime*1) >= maxBossTimer){
				hero.floor--;
				canProgress = false;
			  }
			}
			let lastOfflineTime = new Decimal(offlineTimeReward).sub(killingTime);
			if(lastOfflineTime.greaterThanOrEqualTo(0)){
			  if(canProgress){
		...
		let totalLaborPrize = new Decimal(offlineTime).div(10).mul(calcTotalProduceLaborCoin());
		...
		componentAction ("offline-text", "replaceText", offlineText);
  ...
  var calcCurrentPartnerDps = function(partnerId,battleType=""){
	...
	if(unlockPartner){
	  if(battleType==""){
		let partnerData = partner.find(item => item.id === partnerId);
		return new Decimal(unlockPartner.dps).plus(unlockPartner.bonusDps).plus(highestUnlockPartner.baseDps).sub(partnerData.baseDps);
	  }else return new Decimal(unlockPartner.bonusDps).plus(highestUnlockPartner.baseDps);
	}else return new Decimal(0);
  };
  ...
  var calcNextPartnerDps = function (partnerId,level=0){
	...
	for(let a=0; a<loopNextLevel; a++){
	  ...
	  if((currentPartnerLevel + a + 1) / 10 == 1 || (currentPartnerLevel + a + 1) % 25 == 0){
		totalNextPartnerDps = totalNextPartnerDps.plus((currentPartnerLevel + a + 1)*1.5);
	  }
	}
	return totalNextPartnerDps;
  };
  ...
  var calcNextLaborPrize = function (laborId,level=0){
	...
	for(let a=0; a<loopNextLevel; a++){
	  let nextLaborPrize = new Decimal(unlockLabor.basePrize).mul(currentLaborLevel+a+1);
  ...
  var calcTotalPartnerDps = function(partnerId=-1,type,isLoopAction=false,critHit=false){
	...
	if(highestUnlockPartner){
	  switch(partnerData.type){
	    case "Melee":
		  bonusPartnerTypePercentage = new Decimal(bonusPartnerTypePercentage).plus(hero.bonusMeleeDpsPercentage).plus(hero.companionMeleeDpsPercentage).plus(highestUnlockPartner.enhanceDpsPercentage).div(100);
		  bonusPartnerTypeCritPercentage = new Decimal(bonusPartnerTypeCritPercentage).plus(hero.bonusMeleeCritDamagePercentage);
		  break;
	    case "Stealth":
		  bonusPartnerTypePercentage = new Decimal(bonusPartnerTypePercentage).plus(hero.bonusStealthDpsPercentage).plus(hero.companionStealthDpsPercentage).plus(highestUnlockPartner.enhanceDpsPercentage).div(100);
		  bonusPartnerTypeCritPercentage = new Decimal(bonusPartnerTypeCritPercentage).plus(hero.bonusStealthCritDamagePercentage);
		  break;
	    case "Sword":
		  bonusPartnerTypePercentage = new Decimal(bonusPartnerTypePercentage).plus(hero.bonusSwordDpsPercentage).plus(hero.companionSwordDpsPercentage).plus(highestUnlockPartner.enhanceDpsPercentage).div(100);
		  bonusPartnerTypeCritPercentage = new Decimal(bonusPartnerTypeCritPercentage).plus(hero.bonusSwordCritDamagePercentage);
		  break;
	    case "Ranged":
		  bonusPartnerTypePercentage = new Decimal(bonusPartnerTypePercentage).plus(hero.bonusRangedDpsPercentage).plus(hero.companionRangedDpsPercentage).plus(highestUnlockPartner.enhanceDpsPercentage).div(100);
		  bonusPartnerTypeCritPercentage = new Decimal(bonusPartnerTypeCritPercentage).plus(hero.bonusRangedCritDamagePercentage);
		  break;
	    case "Magic":
	 	  bonusPartnerTypePercentage = new Decimal(bonusPartnerTypePercentage).plus(hero.bonusMagicDpsPercentage).plus(hero.companionMagicDpsPercentage).plus(highestUnlockPartner.enhanceDpsPercentage).div(100);
		  bonusPartnerTypeCritPercentage = new Decimal(bonusPartnerTypeCritPercentage).plus(hero.bonusMagicCritDamagePercentage);
		  break;
	    case "Firearm":
		  bonusPartnerTypePercentage = new Decimal(bonusPartnerTypePercentage).plus(hero.bonusFirearmDpsPercentage).plus(hero.companionFirearmDpsPercentage).plus(highestUnlockPartner.enhanceDpsPercentage).div(100);
		  bonusPartnerTypeCritPercentage = new Decimal(bonusPartnerTypeCritPercentage).plus(hero.bonusFirearmCritDamagePercentage);
		  break;
	    case "Rune":
		  bonusPartnerTypePercentage = new Decimal(bonusPartnerTypePercentage).plus(hero.bonusRuneDpsPercentage).plus(hero.companionRuneDpsPercentage).plus(highestUnlockPartner.enhanceDpsPercentage).div(100);
	  ...
	  if(isLoopAction){
	    ...
	    if(randomPercentage<=partnerPercentage || critHit == true){
		  ...
		  unlockPartner.critHit = true;
	 	  bonusPartnerTypePercentage = new Decimal(highestUnlockPartner.baseCritDamagePercentage).plus(highestUnlockPartner.enhanceCritDamagePercentage).plus(bonusPartnerTypeCritPercentage).div(100).mul(bonusPartnerTypePercentage);
		  componentAction ("current-partner-dps-"+partnerId, "addClass", "unlock-info");
	    }else{
		  unlockPartner.critHit = false;
		  componentAction ("current-partner-dps-"+partnerId, "removeClass", "unlock-info");
		  $("#current-partner-dps-"+partnerId).css({"color":""});
	    }
	  }
	  totalPartnerDps = totalPartnerDps.mul(bonusPartnerTypePercentage).ceil();
	}
  ...
  var runCarriedCoin = function(lastCoin){
	...
	if(hero.bonusChanceCarriedCoinPercentage > 0) carriedCoin = new Decimal(hero.bonusChanceCarriedCoinPercentage).mul(lastCoin).div(100).floor();
	return carriedCoin;
  };
  ...
  var formulaSlashDamage = function(){
	return new Decimal(calcTotalDpc()).plus(calcTotalDps()).mul(2).floor();  
  };
  ...
  var hitCurrentDps = function(){
	...
	if(hero.bonusChanceHitCurrentDpsPercentage > 0){
	  ...
	  if(countHitCurrentDps == 0){
	    hitDamage = new Decimal(calcTotalDps()).mul(hero.bonusChanceHitCurrentDpsPercentage).div(100).floor();
		if(dungeonBattleLog == "") dungeonBattleLog += godsData.name;
		else dungeonBattleLog += ", " + godsData.name;
		countHitCurrentDps++;
	  }
  ...
  var runBillionaire = function (skillId) {
	...
	let rewardCoin = calcTotalRewardCoin();
	rewardCoin = rewardCoin.mul(skillDamage).div(100);
  ...
  var hideAllMenu = function(){
	if ($("#info-status-div").hasClass("show-menu")) componentAction ("info-status-div", "updateClass", "show-menu,hide-menu");
	...
	if ($("#gameplay-status-div").hasClass("show-menu")) componentAction ("gameplay-status-div", "updateClass", "show-menu,hide-menu");
	...
	if ($("#town-status-div").hasClass("show-menu")) componentAction ("town-status-div", "updateClass", "show-menu,hide-menu");
	...
	if ($("#other-status-div").hasClass("show-menu")) componentAction ("other-status-div", "updateClass", "show-menu,hide-menu");
  ...
  var showMenu = function(menuName){
	switch(menuName){
	  ...
	  case 'gameplay':
		...
		componentAction ("tower", "removeClass", "d-none");
		componentAction ("daily-task", "removeClass", "d-none");
		componentAction ("daily-check-in", "removeClass", "d-none");
		componentAction ("artifact", "removeClass", "d-none");
		break;
	  case 'town':
	    componentAction ("town-status-div", "updateClass", "hide-menu,show-menu");
		componentAction ("barracks", "removeClass", "d-none");
		componentAction ("companion", "removeClass", "d-none");
		componentAction ("house", "removeClass", "d-none");
		componentAction ("bank", "removeClass", "d-none");
		componentAction ("pet", "removeClass", "d-none");
		componentAction ("warehouse", "removeClass", "d-none");
		componentAction ("gods", "removeClass", "d-none");
		break;
	  case 'other':
  ...
  $("#skip-story-ok-button").on('click', function(e) {
	$("#modalConfirmSkipStory").modal("toggle");
	skipStory();
  });
  ...
  $("#continue-prestige-ok-button").on('click', function() {
	$("#modalPrestigeForm").modal('toggle');	
	hideAllContent();
	showArtifactArea();
  });
  ...
  $("#notif-features-ok-button").on('click', function() {
	...
	$("#modalNotifNewFeatures").modal('toggle');
	hideAllContent();
	hideAllMenu();
	if(featureName == "tower") showMenu("gameplay");
	else showMenu("town");
	switch(featureName){
	  case "labor":
  ...
  $("[id^=setting-confirm-use-item-]").on('click', function() {
	if($(this).val() == "true") hero.settingConfirmUseItem = true;
  ...
  $("#unlock-artifact-ok-button").on('click', function() {
	$("#modalUnlockArtifactConfirm").modal("toggle");
  ...
  $("#action-item-ok-button").on('click', function() {
	$("#modalConfirmActionItem").modal('toggle');
  ...
  $("#notification-ok-button").on('click', function() {
	...
	if($("#notification-ok-button").val() == "true") location.reload();
	else if($("#notification-ok-button").val() == "checkIn") checkCooldownAdventureTimer(true);
  });
  ...
  $("#tower-image-info").on('click', function(e) {
	$("#modalNotification").modal("show");
	componentAction ("notification-text", "replaceText", "Partner level in the dungeon will not be calculated in the tower");  
  });
