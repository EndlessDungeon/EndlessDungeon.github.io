(function($) {
  ...
  var parseData = function(value){
	let usedKeyLen = value.split("|").pop();
	let usedKeyData = value.slice(0,usedKeyLen);
	
	let data = [];
	data.push(usedKeyData);
	data.push(value.slice(usedKeyLen).slice(0,-3));
	return data;
  };
  ...
  var loadGame = function () {
	...
	if(localStorage.getItem("hero")){
	  ...
	  if(setting.devPage && localStorage.getItem("hero-dev")){
		data = parseData(localStorage.getItem("hero-dev"));
		hero = JSON.parse(decryptData(data[1],data[0]));
	  }
  ...
  var exportData = function (isDownload=false){
	let exportSaveData = '';
	exportSaveData = encryptData(JSON.stringify(hero));
  ...
  var importData = function (){
	...
	try {
	  ...
	  try{
		decryptSaveData = JSON.parse(decryptData(importData,keyData));  
	  }catch(e){
		decryptSaveData = "";
		currentEnemyHP = initDungeon();
		startAllTimer();
  ...
  var localSaveGame = function () {
	...
	let heroData = keyData+encryptData(JSON.stringify(hero))+"|"+keyLen;
  ...
  var initKey = function(){
	keyLen = randomBetweenNumber(30,50);  
  ...
  var calculateAllArtifactDamage = function(firstRun=false){
	...
	artifactDamage = new Decimal(hero.countArtifact).div(5).floor().mul(1).mul(artifactDamage);
  ...
  var updateInGameDateTime = function(){
	...
	let date = new Date();
	let currentDate = ('0' + date.getDate()).slice(-2)+" "+monthName[date.getMonth()]+" "+date.getFullYear();
	let currentTime = ('0' + date.getHours()).slice(-2) + ":" + ('0'+date.getMinutes()).slice(-2) + ":" + ('0'+date.getSeconds()).slice(-2);
	...
	for(let i=0; i<skill.length; i++){
	  ...
	  if(unlockReqFloor >= skill[i].unlockReqFloor){
		...
		if( !$( "#skill-area-" + i ).length){
		  ...
		  appendSkillDiv += '<button id="use-ticket-button-' + i + '" class="upgrade-level-button v-center-flex middle-content" value="' + i + '"><img id="ticket-image-'+i+'" src="assets/images/' + ticketImage + '.svg" class="image-size-2"></img>&nbsp;Use Ticket</button>';
		  appendSkillDiv += '</div>';
	...
	if(hero.highestFloor >= unlockNewFeatureFloor("cave")){
	  if(hero.countArmy > 0){
	    ...
	    if( !$( "#cave-area" ).length){
		  ...
		  contentQuestDiv += '<span>Please ask bartender in tavern to start the quest</span>&nbsp;<button id="meet-bartender" class="upgrade-level-button w-auto">Go To Tavern</button>';
		  contentQuestDiv += '</div>';
  ...
  var calculateTokenTowerKey = function(totalTowerKey,checkedMax=false){
	...
	if(checkedMax){  
	  let maxTowerKey = 0;
	  for(let i=1;i<=99;i++){
		if(i > hero.dailyBuyTowerKey){
		  let currentToken = i * setting.baseTokenBuyTowerKey;  
		  let nextToken = new Decimal(totalToken).plus(currentToken);		  
		  if(heroToken.lessThan(nextToken)){
			maxTowerKey = i-1-hero.dailyBuyTowerKey;
			i = 100;
		  }else totalToken = nextToken;
	    }  
	  }
	  return totalToken+"="+maxTowerKey;
	}else{
	  let maxLoop = totalTowerKey + (hero.dailyBuyTowerKey * 1);
	  for(let i=1;i<=maxLoop;i++){
	    if(i > hero.dailyBuyTowerKey){
		  let currentToken = i * setting.baseTokenBuyTowerKey;  
		  totalToken += currentToken;
		  if(new Decimal(totalToken).greaterThan(heroToken)){
			break;
		  }
	    }
	  }
	  ...
	  if(highestUnlockPartner.ability.length > 0){
		...
		appendDetailPartner = '<div id="detail-partner-ability-area" class="row col-xl-12 col-lg-12 col-md-12 col-sm-12 col-12 px-0">';
		  for(let a=0;a<highestUnlockPartner.ability.length;a++){
			...
			if(showAccuracy){
			  appendDetailPartner += '<button id="acc-all-minus-'+partnerId+'-'+abilityId+'" class="down-count partner-ability-stat btn-info" title="Down" value="'+partnerId+'='+abilityId+'=acc=allMinus">All</button>';
			...
			if(showAccuracy){
			  ...
			  appendDetailPartner += '<button id="acc-all-plus-'+partnerId+'-'+abilityId+'" class="up-count partner-ability-stat btn-info" title="Up" value="'+partnerId+'='+abilityId+'=acc=allPlus">All</button>';
			}
			appendDetailPartner += '</div>';
			appendDetailPartner += '<div class="col-xl-6 col-lg-6 col-md-6 col-sm-6 col-12 v-center-flex my-1">Damage : ';
			appendDetailPartner += '<button id="dam-all-minus-'+partnerId+'-'+abilityId+'" class="down-count partner-ability-stat btn-info" title="Down" value="'+partnerId+'='+abilityId+'=dam=allMinus">All</button>';
			...
			appendDetailPartner += '<button id="dam-all-plus-'+partnerId+'-'+abilityId+'" class="up-count partner-ability-stat btn-info" title="Up" value="'+partnerId+'='+abilityId+'=dam=allPlus">All</button>';
			appendDetailPartner += '</div>';
  ...
  var checkCooldownQuestTimer = function(){
	let countArmy = hero.unlockArmy.length;
	for(let i=0;i<countArmy;i++){
	  if(hero.unlockArmy[i].caveId >= 0) startQuestTimer(hero.unlockArmy[i].id);
	}
  };
  ...
  var startQuestTimer = function(armyId){
	...
	if(questTimer > 0){
	  ...
	  if(!objQuestInterval){
	    questInterval = setTimeout(function loopCooldownQuestTimer() {
	      ...
	      if(questTimer <= 0) stopQuestTimer(armyId);
		  else{
	...
	}else stopQuestTimer(armyId);
	saveGame();
  };
  
  var stopQuestTimer = function(armyId){
	...
	}else questResult = "Sorry, your army named "+unlockArmy.name+" was died in "+caveData.name+" quest";
	updateCampArmyStatus(armyId);
	updateCaveStatus(caveId);
	updateCaveUsedSlot(caveId,"-");
	saveGame();
	componentAction("notification-text","replaceText",questResult);
	$("#modalNotification").modal("show");
  };
	...
	if(unlockItem){
	  ...
	  if(canProcess){
	    ...
	    $("#modal-action-desc").append(appendModalDiv).off('keyup','.clover-sell-qty').on('keyup','.clover-sell-qty',function(e){
		  var isNumeric = e.keyCode || e.which;
		  isNumeric = numericFormat(isNumeric);
		  if(isNumeric){
  ...
  var showDetailPeople = function(selectedName){
	...
	if(selectedName.toLowerCase() == "bartender"){
	  ...
	  appendDetailDialogDiv += '<div class="col-xl-12 col-lg-12 col-md-12 col-sm-12 col-12 m-0">Hello my '+gender+' '+hero.userName+'. May I help you ?</div>';
  ...
  var showCaveContentDiv = function(pageName){
	...
	for(let i=0;i<cave.length;i++){
	  ...
	  contentQuestDiv += '<div class="row col-xl-4 col-lg-4 col-md-4 col-sm-4 col-4">';
	  contentQuestDiv += '<img src="assets/images/'+cave[i].name.replace(" ","-").toLowerCase()+'.svg" class="container-img"></img>';
	  contentQuestDiv += '</div>';
	  contentQuestDiv += '<div class="row col-xl-8 col-lg-8 col-md-8 col-sm-8 col-8">';
	  contentQuestDiv += '<div class="col-xl-12 col-lg-12 col-md-12 col-sm-12 col-12">'+cave[i].name+'</div>';
	  contentQuestDiv += '<div class="col-xl-12 col-lg-12 col-md-12 col-sm-12 col-12 v-center-flex"><img src="assets/images/attack.svg" class="image-size" title="Attack"></img>&nbsp;:&nbsp;'+cave[i].minAtk+'&nbsp;-&nbsp;'+cave[i].maxAtk+'</div>';
	  contentQuestDiv += '<div class="col-xl-12 col-lg-12 col-md-12 col-sm-12 col-12 v-center-flex"><img src="assets/images/defence.svg" class="image-size" title="Defence"></img>&nbsp;:&nbsp;'+cave[i].minDef+'&nbsp;-&nbsp;'+cave[i].maxDef+'</div>';
	  contentQuestDiv += '</div>';
	  for(let j=0;j<setting.maxCaveOpenSlot;j++){
  ...
  var recalculateAllUnlockLabor = function(){
	...
	for(let i=0;i<hero.unlockLabor.length;i++){
	  ...
	  if(hero.unlockLabor[i].baseCost) delete(hero.unlockLabor[i].baseCost);
	  hero.unlockLabor[i].baseFee = baseLaborFee;
	  hero.unlockLabor[i].basePrize = baseLaborPrize;
	  
	  if(hero.unlockLabor[i].grade && hero.unlockLabor[i].grade > 1){
		for(let a=1;a<hero.unlockLabor[i].grade;a++){
	      let enhanceData = calcEnhanceLaborFee(i);
	      baseLaborFee = new Decimal(enhanceData["enhanceFee"]);
	      baseLaborPrize = new Decimal(enhanceData["enhancePrize"]);
		  hero.unlockLabor[i].baseFee = baseLaborFee;
		  hero.unlockLabor[i].basePrize = baseLaborPrize;
		  if(hero.unlockLabor[i].level == 1) hero.unlockLabor[i].currentPrize = hero.unlockLabor[i].basePrize;
		  else{
			let currentPrize = new Decimal(0);
			for(let b=1;b<=hero.unlockLabor[i].level;b++){
			  currentPrize = new Decimal(b).mul(hero.unlockLabor[i].basePrize).plus(currentPrize);
			}
			hero.unlockLabor[i].currentPrize = currentPrize;
		  }
		}
	  }else hero.unlockLabor[i].currentPrize = hero.unlockLabor[i].basePrize;  
	}		
  };
  ...
  var formulaRewardCoin = function(floorNumber){
	let rewardCoin = new Decimal(floorNumber).mul(enemy.baseCoin);
  ...
  var calcTotalOrbs = function (){
	let baseOrbs = new Decimal((hero.maxFloor - unlockNewFeatureFloor("artifact"))/setting.prestigeFloorBonus).plus(setting.baseGainOrbsCalculation);
	baseOrbs = baseOrbs.mul(1+((hero.maxFloor-unlockNewFeatureFloor("artifact"))/50)).pow(setting.orbsMultiplier);
	let totalLevel = new Decimal(calcTotalPartnerLevel()).plus(hero.heroLevel).plus(hero.weaponLevel);
	let levelMultiplier = totalLevel.div(setting.prestigePartnerLevelBonus);
  ...
  $("#buy-sell-qty").keyup(function(e) {
	...
	let isNumeric = numericFormat(value);
  ...
  $("#setting-warning-percentage-hp").keypress(function(e) {
	var value = e.keyCode || e.which;       
	return numericFormat(value);
  });
  
  $("#setting-danger-percentage-hp").keypress(function(e) {
	var value = e.keyCode || e.which;       
	return numericFormat(value);
  });
  ...
  $("#start-quest-ok-button").on('click', function() {
	...
	calcDefaultModalWidth();
	if(parseInt($("#start-quest-army-id").val(),10) >= 0) startQuest($("#start-quest-cave-id").val(),$("#start-quest-army-id").val());
	else{
	  $("#modalNotification").modal("show");
	  componentAction ("notification-text", "replaceText", "You must choose the army before start the quest.");
  ...
  $("#action-item-ok-button").on('click', function() {
	...
	let actionType = arrayValue[1];
	let sellQty = 1;
	if($("#modal-sell-qty-"+arrayValue).length) sellQty = $("#modal-sell-qty-"+arrayValue).val();
  ...
  $("#notification-ok-button").on('click', function() {
	...
	switch(notifValue[0]){
	  case "true":
		...
		break;
	  case "checkIn":
	    checkCooldownAdventureTimer(true);
		break;
	  case "btk":
	    $("#modalBuyTowerKey").modal("show");
		break;
  ...
  $("#artifact-damage-info").on('click', function(e) {
	$("#modalNotification").modal("show");
	componentAction ("notification-text", "replaceText", "* Each artifact will produce artifact power \n * Artifact power will increase the power of DPC, DPS and Gold gaining from killing monster");
  });
